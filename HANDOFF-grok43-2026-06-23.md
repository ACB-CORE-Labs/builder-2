...
CORE smoke not run as builder-II session gate (independent repo; harness invokes CORE on demand via `builder verify`).
...
|---|---|---|
| `builder_ii/*` | created | Platform package: config, backends, routing, harness, benchmark, compliance, goose_launcher, cli |
...
| `pyproject.toml` | modified | builder entrypoint; removed goose-ai stub dep |
...
4. **Init artifact:** Goose recipe `recipes/core-coding.yaml` + `builder_ii/init_content.py` single-source prompt (~175 tokens).
5. **builder-II smoke only** for platform commits; CORE tests via `builder verify <module>`.

---

## What Must Not Be Forgotten

Do not `pip install goose-ai` — it is an unrelated stub. Install **Codename Goose** with `brew install block-goose-cli`. Run `builder start` from builder-II; it loads the CORE recipe and points Goose at the local MLX server.
...
2. Run `builder benchmark` with backend up to capture TTFT/tok/s evidence.
