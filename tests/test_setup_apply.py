    ...
    overlay = {
        ...
        "path_policy": {
            "declared_setup_scopes": {
                "builder_repo": str(tmp_path / "builder"),
    ...
    states = [
        {
            ...
            "change_kinds": [c["change_kind"]],
            "planned_operation_types": [c["operation_type"]],
            ...
            "snapshot_only": True,
            "artifact_is_authority": False,
            "future_rollback_operation_needed": "delete_future_created_path",
            "path_notes": [],
        }
        for c in changes
    ]
    snap = {
        "kind": "builder_ii.setup_rollback_snapshot",
        "schema_version": 1,
        "artifact_is_authority": False,
        "snapshot_only": True,
        ...
        "snapshot_id": "b" * 64,
        "target_paths_covered": [c["target_path"] for c in changes],
        "target_path_states": states,
        "prior_content_default_storage_policy": "x",
        "secret_policy": {
...
def test_apply_denied_without_or_wrong_digest(tmp_path):
    ...
    assert (
        runner.invoke(
            setup_app, ["apply", str(op), "--rollback-snapshot", str(sp), "--output", str(tmp_path / "r.json")]
        ).exit_code
        != 0
    )
    assert (
...
def test_apply_denied_path_traversal_symlink_and_unsupported(tmp_path):
    symlink = tmp_path / "artifacts" / "link.txt"
    (tmp_path / "artifacts").mkdir()
    symlink.symlink_to(tmp_path / "elsewhere")
    for change in [
        _change(tmp_path / "artifacts" / "bad.txt", op="merge"),
        {**_change(symlink), "conflict_classification": "symlink_path"},
        {**_change(tmp_path / "artifacts" / "trav.txt"), "path_traversal_rejected": True},
    ]:
        ...
        assert (
            runner.invoke(
                ...
                [
                    ...
                    overlay["overlay_plan_digest"],
                    "--output",
                    str(tmp_path / f"{change['change_id']}.json"),
                ],
            ).exit_code
            != 0
        )
...
def test_apply_writes_declared_path_receipt_digests_noop_and_redaction(tmp_path):
    ...
    overlay, snap = _artifacts(tmp_path, [secret_change, noop])
    op, sp = _write(tmp_path, overlay, snap)
...
def test_noop_source_repo_paths_are_skipped_not_denied(tmp_path):
    ...
    source.parent.mkdir()
    change = {
        **_change(source, op="no-op"),
        "inside_builder_repo": True,
        "inside_artifact_root": False,
        "path_scope_classification": "builder_repo",
    }
    overlay, snap = _artifacts(tmp_path, [change])
    op, sp = _write(tmp_path, overlay, snap)
    receipt_path = tmp_path / "receipt.json"
