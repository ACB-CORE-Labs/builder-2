...
from builder_ii.setup_rollback_receipt import validate_setup_rollback_receipt_artifact
...
def test_rollback_deletes_future_created_file_and_validates_receipt(tmp_path: Path) -> None:
    ...
    result = runner.invoke(setup_app, ["rollback", str(setup_receipt_path), "--rollback-snapshot", str(sp), "--approve-digest", setup_receipt["receipt_digest"], "--output", str(rollback_receipt_path)])
...
def test_rollback_preflight_denial_does_not_mutate_eligible_prior_path(tmp_path: Path) -> None:
    ...
    setup_receipt, snap, sp = _apply(tmp_path, [_change(path_a, content="A=created\n"), _change(path_b, content="B=created\n")])
    ...
    result = runner.invoke(setup_app, ["rollback", str(tmp_path / "setup-receipt.json"), "--rollback-snapshot", str(sp), "--approve-digest", setup_receipt["receipt_digest"], "--output", str(rollback_receipt_path)])
...
def test_denied_or_failed_apply_receipts_are_not_eligible_for_rollback(tmp_path: Path) -> None:
    ...
    result = runner.invoke(setup_app, ["rollback", str(bad_receipt), "--rollback-snapshot", str(sp), "--approve-digest", setup_receipt["receipt_digest"], "--output", str(out)])
