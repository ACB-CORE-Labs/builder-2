...
def test_prepare_platform_spine_contains_only_open_core_artifacts(tmp_path):
    repo = _make_repo(tmp_path)
    settings = load_settings(project_root=ROOT)
    kernel = ConventionKernel()
...
def test_open_core_does_not_infer_authority_for_plugin_internal_commands():
    """The opaque import seam is registered; unregistered plugin commands are not."""
    ...
    assert not any(getattr(group, flag) for flag in CAPABILITY_FLAGS)
    assert find_matching_record("builder-code-vault opaque-command") is None
...
def test_a_stand_in_record_never_inherits_from_itself():
    ...
    `inheritance_errors` is the single checker the registry is held to; the stand-in record is
    ...
    for command in ("builder-goose frobnicate",):
