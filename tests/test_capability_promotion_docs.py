...
def test_capability_promotion_doc_exists_and_key_artifacts_mentioned() -> None:
    doc_path = (
        Path(__file__).resolve().parent.parent / "docs" / "CAPABILITY_PROMOTION.md"
    )
    ...
    for artifact in key_artifacts:
        assert artifact in content, (
            f"Missing key artifact reference in CAPABILITY_PROMOTION.md: {artifact}"
        )
...
def test_capability_promotion_doc_includes_required_promotion_states() -> None:
    ...
    for state in required_states:
        assert f"`{state}`" in content or state in content, (
            f"Missing required promotion state: {state}"
        )
...
def test_capability_promotion_doc_avoids_forbidden_identity_framing() -> None:
    ...
    assert "CORE builder-II" not in content, (
        "Forbidden global identity framing found: 'CORE builder-II'"
    )
...
def test_capability_promotion_doc_states_unpromoted_boundaries() -> None:
    ...
    assert (
        "deepagents active delegation" in content
        or "Deepagents active delegation" in content
    ), "Must mention deepagents active delegation"
    assert "forbidden_unpromoted" in content, (
        "Must indicate that active runtimes are forbidden/unpromoted"
    )
...
def test_capability_promotion_doc_covers_goal2_assignment_surface() -> None:
    ...
    for phrase in required_phrases:
        assert phrase in content, (
            f"Missing Goal 2 capability promotion phrase: {phrase}"
        )
