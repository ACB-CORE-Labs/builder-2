...
def test_chain_accepts_verification_execution_ledger_reconstruction_report(tmp_path: Path) -> None:
    artifact_root = tmp_path / ".builder" / "verification"
    artifact_root.mkdir(parents=True, exist_ok=True)
    ledger_root = tmp_path / ".builder" / "ledger"
    ...
    record_path = ledger_root / "verification-execution-ledger.json"
    write_verification_execution_ledger_record(record, record_path)
    reconstruction_report = reconstruct_verification_execution_ledger(ledger_root=ledger_root)
    report_path = tmp_path / "verification-execution-ledger-reconstruction-report.json"
    report_path.write_text(json_lib.dumps(reconstruction_report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
