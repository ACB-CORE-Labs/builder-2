...
expose no CodeVault capability it cannot back, and must exit non-zero.

The refusal must also say *why*. Only naming CodeVault distinguishes a severed capability
from a missing one. Nothing else in the suite covers the fallback branch, so
...
OPAQUE_ARGUMENT_SAMPLES = [[], ["anything"], ["opaque", "--flag"]]
...
def test_absent_plugin_registers_no_code_vault_capability(shim_without_plugin) -> None:
    assert len(shim_without_plugin.code_vault_app.registered_commands) == 1


@pytest.mark.parametrize("argv", OPAQUE_ARGUMENT_SAMPLES)
