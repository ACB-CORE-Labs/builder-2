...
def test_valid_rollback_approval_validates() -> None:
    plan = _plan()
    approval = create_hitl_rollback_approval(plan, confirmed_digest_prefix=canonical_json_digest(plan)[:4])
    ...
    assert approval["rollback_plan_digest"] == canonical_json_digest(plan)
...
def test_binding_holds_for_matching_plan() -> None:
    ...
    assert (
        rollback_approval_binding_errors(
            approval, rollback_plan_digest=canonical_json_digest(plan), patch_digest="a" * 64
        )
        == []
    )
...
def test_binding_fails_for_tampered_plan() -> None:
    ...
    errors = rollback_approval_binding_errors(
        approval, rollback_plan_digest=canonical_json_digest(tampered), patch_digest="a" * 64
    )
...
def test_binding_fails_for_wrong_patch_digest() -> None:
    ...
    approval = create_hitl_rollback_approval(plan, confirmed_digest_prefix=canonical_json_digest(plan)[:4])
    errors = rollback_approval_binding_errors(
        approval, rollback_plan_digest=canonical_json_digest(plan), patch_digest="f" * 64
    )
...
def test_expiry_fails_closed() -> None:
    ...
    approval = create_hitl_rollback_approval(
        plan, confirmed_digest_prefix=canonical_json_digest(plan)[:4], approved_at=1000, ttl_seconds=100
    )
