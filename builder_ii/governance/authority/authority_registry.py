...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Reads passive governance artifacts and registries to render a spatial instrument console (chain spine, model/agent matrices, workflow lanes, operator next, platform audit); delegates execution to core tools. Every launch path (`builder stratum`, `builder-stratum`, `builder-platform tui`, `python -m builder_ii.tui`) calls enforce_command_authority against this record before constructing the app; none is exempt. Command tier evaluation is real: it asks check_command_authority rather than guessing from the tier. No chain digest is displayed, because verify_artifact_chain exposes none; the surface renders an explicit absence marker instead of a synthesized value. It starts runtimes only by handing off to governed commands with fixed argv and shell=False, never by acting itself. Three declared invocations: (1) the Goose keybinding suspends the TUI and hands the terminal to builder-goose start-readonly; (2) Ctrl+G collects a task, delegates a passive read_only manifest mint, and spawns builder-goose run-governed in the background without suspending, so the run streams onto the session ledger the cockpit tails; (3) the HITL approve/refuse keys suspend and hand the terminal to builder-hitl approve-patch / refuse-patch. Each command applies its own policy, emits its own receipts, and re-validates its own approvals at its own boundary. It fails closed twice before anything spawns -- enforce_command_authority must permit the governed command, and either a valid read_only manifest already exists or the operator confirms passive auto-prep (else the hand-off aborts). It never spawns Goose directly and never selects Goose builtins. HITL approve/reject still never harvest the digest they render: the direct path suspends and lets builder-hitl print the digest and ask for its prefix in the operator's own terminal, because that typed prefix is the approval evidence itself. The keys are gated on the affordance projection and fall back to composing when the registry does not derive invoke_direct. The HITL diff viewer renders the bound patch proposal's unified diff read-only and applies nothing.",
        write_boundary="Does not write target source, approval state, or HITL digests. The prepare-package and subagent-dispatch keybindings collect operator input and then refuse, naming builder-session prepare-package and builder-deepagents assign-subagent and prefilling the Command Composer. After an explicit ConfirmScreen yes on G, or on Ctrl+G when a standing ratification grant covers stratum.dispatch.goose_run, stratum_prepare may create empty .builder scaffold dirs and one passive read_only Goose session manifest under .builder/goose (local scaffolding); the TUI itself writes no files, and every such write is delegated to a non-TUI module. Ratification of a dispatch is appended to the ratification ledger by the same delegated path. Receipts under .builder/receipts are written by builder-goose start-readonly, never by the render surface.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Operator must explicitly invoke one of STRATUM's launch commands from an active terminal; enforce_command_authority evaluates this record on every one of them before the app is constructed. No keybinding originates authority. Dispatch and HITL keys reach governed commands that evaluate their own authority again and fail closed on their own boundaries; where those keys pause is operator-configured through ratification grants, which relocate the prompt without removing any artifact, receipt, or ledger entry. Patch approval can never be grant-covered: it is registered human_approval_mint precisely so a standing grant cannot manufacture a human decision. The one runtime it starts is started by that governed command in the operator's own terminal, never by the TUI harvesting a confirmation for it. It executes nothing else and claims no execution: the command palette is a tier-permission inspector, and the CLI passthrough is a context-injecting composer that surfaces a command for the operator to run in their own terminal.",
        ...
        failure_mode="Exits non-zero if command authority denies this record, textual is missing, or the UI crashes; target unchanged. A refused or failed governed Goose launch is reported with the command's own exit code and never as success.",
        notes="STRATUM Mastery Resonance operator console. Gated behind the command-authority registry (plan D5 / item 3.13), enforced uniformly across all four launch paths. Tier evaluation is wired to this registry; the chain digest is shown as absent because none is reachable; prepare/subagent/HITL approve-reject are constitutive refusals; G may mint a passive read_only manifest only after ConfirmScreen, then launches builder-goose start-readonly; the HITL diff viewer renders the bound proposal diff read-only.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Console-script alias of `builder stratum`. Same instrument console, same Goose hand-off, same refuse-and-compose boundaries.",
        write_boundary="Matches `builder stratum`: passive operator-confirmed .builder Goose manifest via stratum_prepare only; target source and HITL digests stay off-limits.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Starts nothing. This is a fail-closed redirect: it prints the governed lanes and exits non-zero. It formerly advertised an ungoverned full-capability Goose launch (--with-builtin developer,skills,summon: file editing and shell, with no read-only policy, no launch receipt and no no-mutation postflight) and had been unable to complete a launch for as long as the wrapper-plan drift check existed -- that check reads artifact['argv'], while create_goose_wrapper_plan writes argv under operator_launch.argv, so the lookup is always None and the comparison always raises. Repairing it would have meant reviving exactly the ungoverned runtime that builder-goose start-readonly exists at TIER_3 to replace, so it refuses on the record and names builder-goose start-governed, builder-goose run-governed, and STRATUM Ctrl+G instead.",
        write_boundary="No writes of any kind. The redirect prints and exits; no path through it touches the filesystem, process state, or any artifact.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Explicit operator invocation only; the command refuses before evaluating anything further, so there is no approval chain to satisfy.",
        output_behavior="Prints the governed alternatives and exits 1.",
        failure_mode="Always exits non-zero. This is the success case: the command exists so an operator's existing muscle memory reaches a message naming the working lanes rather than an unknown-command error or an opaque drift failure.",
        notes="Retired launch path (fail-closed redirect), following the R1.4 precedent for legacy setup. Flags are still accepted and ignored so old invocations reach the message rather than a parse error.",
        allows_external_tool_invocation=True,
    ),
    CommandAuthorityRecord(
        name="builder ask",
    ...
    CommandAuthorityRecord(
        name="builder-goose start-governed",
        entrypoint="builder_ii.goose_cli:goose_app",
        tier=TIER_3,
        promotion_state=STATE_READ_ONLY_RUNTIME_CANDIDATE,
        runtime_boundary="Starts a Goose session whose only tool surface is the builder-II governed MCP server (recipes/governed-readonly.yaml, whose sole extension is `builder-mcp serve`). Goose builtins are stripped, so every tool call Goose makes travels the governed envelope -> receipt -> ledger ceremony instead of a native builtin: read-only tools execute and are receipted, mutating tool classes are refused in-loop and ledgered as denied. Like start-readonly it snapshots every target file by content digest before launch and runs a no-mutation postflight on close; unlike start-readonly, which strips builtins and leaves Goose with no tools at all, this run is legible -- it opens the hash-chained session event ledger before the child is spawned and closes it after the child exits, which is the chain the operator console tails.",
        write_boundary="No target source writes. Writes only its own governed evidence: launch and close receipts under .builder/receipts, and the session's hash-chained event ledger plus per-call policy/envelope/receipt sidecars under .builder/sessions/<session_id>. The in-loop patch apply path stays deny-by-default and is not enabled by this command.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Requires an explicit operator invocation naming an existing read_only session manifest; enforce_command_authority evaluates this record before any process is spawned. Starting a governed session grants no authority to what runs inside it -- the MCP server re-derives its own deny-by-default policy per call and refuses mutating tool classes on its own boundary.",
        output_behavior="Outputs launch and close receipts, a no-mutation postflight, and one chained start/close event pair on the session ledger.",
        failure_mode="Exits non-zero when the manifest is missing, unreadable, or not read_only; when command authority denies this record; when the Goose binary is absent; or when the postflight detects any mutation. Fails closed before spawning in every case except a postflight mutation, which is reported after the fact with the offending paths named.",
        notes="Goose in-loop governed runtime candidate (ADR-0009 lane G). Read-only runtime candidate, not an enabled capability: it is the reachable entry point for the governed MCP interposition that already existed but had no command surface.",
        allows_runtime_start=True,
        allows_external_tool_invocation=True,
        allows_artifact_writes=True,
    ),
    CommandAuthorityRecord(
        name="builder-goose run-governed",
        entrypoint="builder_ii.goose_cli:goose_app",
        tier=TIER_3,
        promotion_state=STATE_READ_ONLY_RUNTIME_CANDIDATE,
        runtime_boundary="Runs a governed Goose task headlessly against recipes/governed-readonly.yaml, whose sole extension is `builder-mcp serve`, with Goose builtins stripped. The non-suspending counterpart to start-governed: instead of handing Goose the operator's terminal and blocking, it streams the child's output to .builder/sessions/<session_id>/goose_run.log and brackets the child with goose_run_started / goose_run_completed events on the same hash-chained session ledger the operator console tails, so a run is legible while it happens. Raw model output is never hashed into the chain -- model output is not evidence -- and the log is byte-capped. Headless support is detected from `goose run --help` rather than assumed, and the command fails closed naming start-governed when this Goose build cannot be handed a task. SIGTERM is handled by the wrapper: it records run_stop_requested, escalates TERM then KILL only if the child ignores it, and still runs the no-mutation postflight.",
        write_boundary="No target source writes. Writes only its own governed evidence: launch and close receipts under .builder/receipts, the bounded run log, and the session's hash-chained event ledger with the governed MCP server's per-call sidecars. `--enable-governed-apply` sets BUILDER_MCP_GOVERNED_APPLY for the spawned child only; it unlocks the deny-by-default apply lane rather than permitting any particular write, and that lane still re-validates a digest-bound approval at its own boundary and fails closed without one.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Requires an explicit operator invocation naming an existing read_only session manifest and a task; enforce_command_authority evaluates this record before any process is spawned. Starting a run grants no authority to what runs inside it: the MCP server re-derives its own deny-by-default policy per call, refuses mutating tool classes, and the apply lane demands its own approval.",
        output_behavior="Streams the child's output to a bounded run log, and emits launch/close receipts, a no-mutation postflight, and a chained start/completed event pair on the session ledger.",
        failure_mode="Exits non-zero when the manifest is missing, unreadable, or not read_only; when command authority denies this record; when the Goose binary is absent or cannot run headlessly; when the postflight detects a mutation; or with the child's own exit code when it fails. Fails closed before spawning in every case except a postflight mutation or a child failure, both reported after the fact.",
        notes="Goose in-loop governed runtime candidate (ADR-0009 lane B). Read-only runtime candidate, not an enabled capability. This is the dispatch target a non-suspending operator console needs: the run appears on the ledger the cockpit already tails, with no terminal handover.",
        allows_runtime_start=True,
        allows_external_tool_invocation=True,
        allows_artifact_writes=True,
    ),
