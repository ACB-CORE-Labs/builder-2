...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    CommandAuthorityRecord(
        ...
        notes="STRATUM Mastery Resonance operator console. Gated behind --experimental (plan D5 / item 3.13). Tier evaluation is wired to this registry; the chain digest is shown as absent because none is reachable; prepare/subagent/HITL approve-reject are constitutive refusals; G may mint a passive read_only manifest only after ConfirmScreen, then launches builder-goose start-readonly; the HITL diff viewer renders the bound proposal diff read-only.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Console-script alias of `builder stratum --experimental`. Same instrument console, same Goose hand-off, same refuse-and-compose boundaries.",
        ...
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Explicit operator invocation of the console script. Authority and experimental gate match `builder stratum`.",
        output_behavior="Takes over terminal with Textual TUI interface; suspends it while the governed read-only Goose command holds the terminal.",
        failure_mode="Exits non-zero if textual is missing or the UI crashes; target unchanged.",
        notes="Convenience entrypoint equivalent to `builder stratum --experimental`. Registered so pyproject console scripts are fully covered by the authority registry. Same capability flags as `builder stratum`.",
        allows_external_tool_invocation=True,
        allows_runtime_start=True,
        allows_artifact_writes=True,
    ),
