...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_ARTIFACT_ONLY,
        runtime_boundary="Records completed harness results, exact source/wheel identity, runtime/tool versions, elapsed time, skips, copied log digests, and lane-specific claims; executes no proof command itself.",
        write_boundary="Writes one operator-selected typed release evidence JSON artifact plus copied log constituents.",
        approval_mode=MODE_NONE,
        approval_boundary="The invoking harness owns execution authority; this command validates and records completed results but cannot make an unrun command pass.",
        ...
        notes="Lane-specific validation and later bundle cross-binding are mandatory; evidence recording is not promotion, tag, or publication authority.",
        allows_artifact_writes=True,
    ),
    CommandAuthorityRecord(
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Projects one exact governed run from the admitted artifact root; watch mode polls only canonical run evidence and launches no process.",
        write_boundary="No changes to workspace, runtime state, or target repository.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Explicit operator invocation only.",
        output_behavior="Prints the goal/now/needs-you/next/proof run view or its frontend-neutral JSON payload.",
        failure_mode="Exits non-zero for an unknown explicit run, invalid configuration, or corrupt selected evidence without falling back to another run.",
        notes="Read-only run projection. Environment, Goose, backend, recipe, compliance, and model diagnostics belong to builder doctor.",
    ),
    CommandAuthorityRecord(
        name="builder chain",
