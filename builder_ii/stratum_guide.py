...
WALKTHROUGH_STEPS: tuple[WalkthroughStep, ...] = (
    ...
    WalkthroughStep(
        ...
        command="uv run builder-session prepare-package generic -o .builder/session --task \"first stratum session\"",
    ...
    WalkthroughStep(
        ...
        command="uv run builder-session validate-prepare-package .builder/session",
...
def help_keymap_lines() -> list[str]:
    return [
        ...
        "  B         CodeVault frames / demo compose",
        ...
        "  A / R     HITL: compose approve-patch / rejection-record (no mutation)",
        "  I         HITL: bind pending proposal or inspect payload",
        "  D         HITL diff (not implemented)",
        "",
...
def help_boundary_lines() -> list[str]:
    return [
        ...
        "  CodeVault: builder-code-vault demo / validate-demo (exact recall, not ANN)",
    ]
