...
from builder_ii.context_pack import validate_context_pack_record_file
...
def validate_context_summary(record: Any) -> list[str]:
    ...
    for f in ("source_paths", "source_hashes", "target_profile", "model_alias", "model_backend", "prompt_used", "summary", "known_omissions", "claim_boundary"):
        if f not in record:
...
def summarize_context_pack(
    ...
    *,
    ...
    envelope_path = temp_dir / f"summary_env_{int(time.time())}.json"
    receipt_path = temp_dir / f"summary_rec_{int(time.time())}.json"
    ...
    source_hashes = {}
    repo_root = Path(record_data.get("markdown_path", ".")).parent
    ...
    # Cleanup temp envelope and receipt
    ...
    if output_summary_path is not None:
        ...
        output_summary_path.write_text(json_lib.dumps(summary_artifact, indent=2, sort_keys=True) + "\n", encoding="utf-8")
