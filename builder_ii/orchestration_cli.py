...
orchestration_app = typer.Typer(
    help="Create and validate governed agent orchestration plan artifacts."
)
console = Console()
...
def plan_orchestration(
    target: str = typer.Argument(
        ..., help="Target profile name: generic | builder | core"
    ),
    task: str = typer.Option("", "--task", help="Task description"),
    roles: Optional[str] = typer.Option(
        None, "--roles", help="Comma-separated agent role sequence"
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Write JSON artifact to this path"
    ),
) -> None:
    ...
    try:
        ...
        if parsed_roles is None:
            ...
            plan = create_orchestration_plan(
                target=target_norm, task=task, roles=parsed_roles
            )
...
def validate_orchestration(
    ...
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Write Goal 2 validation report JSON to this path"
    ),
) -> None:
    ...
    if data.get("kind") == ORCHESTRATION_PLAN_KIND:
        ...
        if data.get("kind") in {
            ...
            report = create_orchestration_assignment_validation_report(
                data, subject_path=path
            )

    if output is not None and report is not None:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(
            json_lib.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
    if errors:
        for error in errors:
...
def render_assignment(
    ...
    try:
        assignment = create_agent_assignment_plan(
            ...
            profile_pack_validation_report=_read_json(
                profile_pack_validation_report_path
            ),
    ...
    try:
        plan = create_orchestration_assignment_plan(
            assignment, assignment_plan_path=assignment_path
        )
    except ValueError as exc:
        console.print(f"[red]{exc}[/]")
        raise typer.Exit(1)
...
def dry_run_assignment(
    ...
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Write dry-run JSON to this path"
    ),
) -> None:
    ...
    try:
        dry_run = create_orchestration_assignment_dry_run(
            plan, orchestration_assignment_plan_path=plan_path
        )
    except ValueError as exc:
        console.print(f"[red]{exc}[/]")
        raise typer.Exit(1)
