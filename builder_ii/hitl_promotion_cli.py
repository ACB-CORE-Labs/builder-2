...
def promotion_request(
    proposal_path: Path = typer.Option(
        ..., "--proposal-path", help="Path to Goal 2 or Goal 3 proposal artifact"
    ),
    output: Path = typer.Option(
        ..., "--output", help="Output path for the promotion request JSON"
    ),
    requested_by: str = typer.Option(
        "operator", "--requested-by", help="Operator or agent requesting promotion"
    ),
    ...
    if session_manifest_path is not None:
        ...
        session_ref = _create_ref(
            sdata, path=session_manifest_path, role="session_manifest"
        )
...
def promotion_review(
    request_path: Path = typer.Option(
        ..., "--request-path", help="Path to promotion request artifact"
    ),
    output: Path = typer.Option(
        ..., "--output", help="Output path for review artifact"
    ),
    ...
    reviewed_by: str = typer.Option(
        "operator", "--reviewed-by", help="Reviewer identity"
    ),
    recommendation: str = typer.Option(
        "", "--recommendation", help="Recommendation summary"
    ),
    finding: list[str] = typer.Option(
        None, "--finding", help="Review finding (can be repeated)"
    ),
    warning: list[str] = typer.Option(
        None, "--warning", help="Review warning (can be repeated)"
    ),
    blocking_issue: list[str] = typer.Option(
        None, "--blocking-issue", help="Blocking issue (can be repeated)"
    ),
    policy_path: Path | None = typer.Option(
        None, "--policy-path", help="Optional path to policy artifact"
    ),
) -> None:
...
def promotion_decision(
    ...
    review_path: Path = typer.Option(
        ..., "--review-path", help="Path to promotion review artifact"
    ),
    output: Path = typer.Option(
        ..., "--output", help="Output path for decision artifact"
    ),
    ...
    decided_by: str = typer.Option(
        "operator", "--decided-by", help="Operator making decision"
    ),
    reason: str = typer.Option("", "--reason", help="Decision rationale"),
    blocker: list[str] = typer.Option(
        None, "--blocker", help="Unresolved blocker (can be repeated)"
    ),
) -> None:
...
def approval_boundary(
    decision_path: Path = typer.Option(
        ..., "--decision-path", help="Path to promotion decision artifact"
    ),
    request_path: Path = typer.Option(
        ..., "--request-path", help="Path to promotion request artifact"
    ),
    output: Path = typer.Option(
        ..., "--output", help="Output path for approval boundary artifact"
    ),
    allowed_profile: list[str] = typer.Option(
        None, "--allowed-profile", help="Allowed profile name (can be repeated)"
    ),
    ...
    denied = (
        denied_boundary
        if denied_boundary
        else ["runtime execution", "memory mutation", "target repo writes"]
    )
...
def rejection_record(
    ...
    output: Path = typer.Option(
        ..., "--output", help="Output path for rejection record"
    ),
    rationale: str = typer.Option(..., "--rationale", help="Rejection rationale"),
    rejected_by: str = typer.Option(
        "operator", "--rejected-by", help="Operator identity"
    ),
    decision_path: Path | None = typer.Option(
        None, "--decision-path", help="Optional path to decision artifact"
    ),
) -> None:
...
def validate_promotion(
    paths: list[Path] = typer.Argument(
        ..., help="Paths to promotion bridge artifact JSON files"
    ),
) -> None:
