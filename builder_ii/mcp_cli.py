...
def call(
    ...
    receipt_output: Optional[Path] = typer.Option(None, "--receipt-output", "-r", help="Path to save the receipt"),
    session_id: str | None = typer.Option(None, "--session-id", help="Session ID for the operational ledger event"),
) -> None:
    """Executes an MCP call defined in an envelope, validated against a policy."""
    try:
    ...
    if session_id:
        ...
        rec_ref = _artifact_ref(receipt, receipt_output or Path("receipt.json"), "mcp_call_receipt")
