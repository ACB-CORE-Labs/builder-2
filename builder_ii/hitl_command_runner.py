...
FORBIDDEN_COMMANDS = {
    "rm", "git push", "git commit", "git reset", "mv", "chmod", "chown",
    "sudo", "su", "docker exec", "curl", "wget", "nc"
}
...
def _validate_safety(command_str: str) -> None:
    ...
    if cmd_base in FORBIDDEN_COMMANDS:
        raise ValueError(f"Command '{cmd_base}' is forbidden by the execution gateway.")
...
def execute_hitl_command(
    ...
    if recorded_digest != proposal_digest:
        raise ValueError(f"Approval record digest '{recorded_digest}' does not match command proposal digest '{proposal_digest}'")
