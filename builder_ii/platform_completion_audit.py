...
REQUIRED_CAPABILITY_ROWS: tuple[CapabilityRow, ...] = (
    ...
    _row(
        "config schema",
        DESIGN_ONLY,
        ("builder_ii/config.py", ".env.example"),
        (),
        ("tests/test_config_models.py", "tests/test_platform_completion_truth.py"),
        (
            "Settings are Python/env parsing rather than a governed builder-II config artifact.",
            "Versioned schema, validator, redaction, source refs, target roots, artifact roots, capability defaults, and migration are missing.",
        ),
        "R1",
    ),
    _row(
        "config source precedence",
        DESIGN_ONLY,
        ("builder_ii/config.py", "README.md"),
        (),
        ("tests/test_config_models.py", "tests/test_platform_completion_truth.py"),
        (
            "Precedence is implicit across env, .env, CLI defaults, profiles, and generated Goose files.",
            "No artifact says which source wins for target repo, artifact root, active profiles, model/backend, or capability state.",
        ),
        "R1",
    ),
    _row(
    ...
    _row(
        "non-interactive setup/apply/validate",
        MERGED_BUT_NOT_OPERATIONAL,
        ("builder_ii/cli.py", "builder_ii/goose_setup.py"),
        ("builder",),
        ("tests/test_goose_setup.py",),
        (
            "Existing setup writes config/hints/skills.",
            "Canonical plan/apply/validate contract, setup receipt, rollback artifact, source precedence record, and authority matrix are missing.",
        ),
        "R1",
    ),
    _row(
        ...
        ("tests/test_goose_setup.py", "tests/scenarios/test_config_to_goose_projection_flow.py"),
        (
            "Merge behavior exists.",
            "Overlay diff, prior config snapshot, rollback artifact, approved write boundary, and audit receipt are missing.",
        ),
        "R1",
    ),
    _row(
    ...
    _row(
        "skill generator/installer/validator",
        MERGED_BUT_NOT_OPERATIONAL,
        (".agents/skills/core-governed-coding/SKILL.md", "builder_ii/goose_setup.py"),
        ("builder",),
        ("tests/test_goose_setup.py",),
        (
            "Legacy skill installer copies into the target repo.",
            "Generated install plan, digest-bound validation, setup receipt, rollback, and target-scoped approval are missing.",
        ),
        "R1",
    ),
    _row(
    ...
    _row(
        "setup receipt + rollback artifact",
        NOT_STARTED,
        ("builder_ii/receipt_records.py", "builder_ii/rollback_artifacts.py"),
        ("builder-receipt",),
        ("tests/test_receipt_records.py", "tests/test_rollback_artifacts.py"),
        (
            "Generic records exist.",
            "Setup preflight snapshot, plan digest, changed-path list, prior-config snapshot, rollback plan, rollback receipt, ledger event, and replay binding are missing.",
        ),
        "R1",
    ),
    _row(
    ...
    _row(
        ...
        ("tests/test_goose_setup.py",),
        (
            "Legacy setup writes Goose config, hints, skills, and recipes.",
            "It is operator-managed behavior, not governed runtime promotion.",
        ),
        "B4 after R0/B3",
    ),
    _row(
...
def render_human_summary(rows: tuple[CapabilityRow, ...] = REQUIRED_CAPABILITY_ROWS) -> str:
    ...
    lines = [
        ...
        "No runtime execution, patch application, model/provider call, MCP/tool invocation, Goose runtime promotion, deepagents runtime, autonomous write, or commit/push authority is promoted by R0.",
    ...
    lines.extend(
        [
            "",
            "R0 truth-machine rows promoted by this PR:",
            "- platform doctor/status/audit: PASSIVE_FOUNDATION",
            "- docs truth enforcement: PASSIVE_FOUNDATION",
            "",
            "Config/onboarding rows exist in the matrix and remain non-operational until R1.",
        ]
    )
    return "\n".join(lines) + "\n"
