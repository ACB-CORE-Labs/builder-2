...
REQUIRED_CAPABILITY_ROWS: tuple[CapabilityRow, ...] = (
    ...
    _row(
        "non-interactive setup/apply/validate",
        MERGED_BUT_NOT_OPERATIONAL,
        ("builder_ii/cli.py", "builder_ii/goose_setup.py", "builder_ii/setup_plan.py", "builder_ii/setup_cli.py"),
        ("builder", "builder-setup"),
        ("tests/test_goose_setup.py", "tests/test_setup_plan.py", "tests/test_config_setup_cli.py"),
        (
            "Legacy builder setup still writes config/hints/skills outside the governed R1.1 lane.",
            "R1.1 adds passive setup plan and validate-plan artifacts only; setup apply, setup receipt, rollback artifact, and governed write authority are missing.",
        ),
        "R1",
    ),
    _row(
        ...
        (
            ...
            "R1.1 can only describe Goose config and recipe target paths in a passive plan; overlay diff, prior config snapshot, rollback artifact, approved write boundary, and audit receipt are missing.",
        ),
        "R1",
    ),
    _row(
...
def render_human_summary(rows: tuple[CapabilityRow, ...] = REQUIRED_CAPABILITY_ROWS) -> str:
    ...
    lines = [
        ...
        "No runtime execution, patch application, model/provider call, MCP/tool invocation, Goose runtime promotion, deepagents runtime, autonomous write, or commit/push authority is promoted by R1.1.",
    ...
    lines.extend(
        [
            "",
            "R1.1 passive config/onboarding rows promoted by this PR:",
            "- config schema: PASSIVE_FOUNDATION",
            "- config source precedence: PASSIVE_FOUNDATION",
            "- setup plan/validate command surface: artifact-only and non-operational",
            "",
            "Setup apply, setup rollback, interactive setup wizard, runtime execution, and B1 verification execution remain non-operational.",
        ]
    )
    return "\n".join(lines) + "\n"
