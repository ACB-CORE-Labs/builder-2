...
from builder_ii.core.config_schema import CAPABILITY_DEFAULTS, attach_digest, digest_jsonable

SETUP_ROLLBACK_RECEIPT_KIND = "builder_ii.setup_rollback_receipt"
SETUP_ROLLBACK_RECEIPT_SCHEMA_VERSION = 1
...
def dumps_setup_rollback_receipt(receipt: dict[str, Any]) -> str:
    return json_lib.dumps(receipt, indent=2, sort_keys=True) + "\n"


def write_setup_rollback_receipt(receipt: dict[str, Any], output: Path) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(dumps_setup_rollback_receipt(receipt), encoding="utf-8")


def finalize_setup_rollback_receipt(receipt: dict[str, Any]) -> dict[str, Any]:
    receipt.setdefault("kind", SETUP_ROLLBACK_RECEIPT_KIND)
    receipt.setdefault("schema_version", SETUP_ROLLBACK_RECEIPT_SCHEMA_VERSION)
    receipt.setdefault("artifact_is_authority", False)
    receipt.setdefault("setup_apply_executed", True)
    receipt.setdefault("rollback_executed", True)
    ...
    receipt.setdefault("git_rollback", "disabled")
    receipt.setdefault("generic_repository_rollback", "disabled")
...
def validate_setup_rollback_receipt_artifact(data: Any) -> list[str]:
    errors: list[str] = []
    if not isinstance(data, dict):
        return ["setup rollback receipt artifact must be a JSON object"]
    if data.get("kind") != SETUP_ROLLBACK_RECEIPT_KIND:
    ...
    if data.get("rollback_executed") is not True:
        errors.append("rollback_executed must be true")
    if data.get("operation_attempted") != "setup_rollback":
    ...
    digest = data.get("rollback_receipt_digest")
    if not _is_sha256(digest):
        errors.append("rollback_receipt_digest must be a SHA-256 hex string")
    elif digest != digest_jsonable(data, digest_key="rollback_receipt_digest"):
        errors.append("rollback_receipt_digest does not match canonical rollback receipt payload")
    return errors


def validate_setup_rollback_receipt_file(path: Path) -> list[str]:
    if not path.exists():
    ...
    return validate_setup_rollback_receipt_artifact(data)
