...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    ...
    CommandAuthorityRecord(
        ...
        approval_mode=MODE_HITL_ARTIFACT_REQUIRED,
        approval_boundary="Requires an exact digest match with a signed human approval artifact.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_3,
        promotion_state=STATE_HITL_RUNTIME_CANDIDATE,
        runtime_boundary="Restores pre-apply working tree using git apply -R on stored forward patch.",
        write_boundary="Modifies source tree via git apply -R only; writes rollback receipt with equivalence proof.",
        approval_mode=MODE_HITL_ARTIFACT_REQUIRED,
        approval_boundary="Requires valid rollback plan JSON with digest-bound forward patch reference.",
        output_behavior="Applies git apply -R forward_patch_for_reverse_apply.patch.",
        failure_mode="Fails closed on digest mismatch, target mismatch, or post-rollback status drift from pre-apply binding.",
        notes="Rollback stores forward patch for reverse apply; proves pre/post status digest equivalence.",
        allows_source_writes=True,
        allows_artifact_writes=True,
    ),
    CommandAuthorityRecord(
