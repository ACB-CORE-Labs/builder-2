...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_READ_ONLY_RUNTIME_CANDIDATE,
        runtime_boundary="Bounded explicit-path content capture with secret redaction; no recursive discovery or glob expansion.",
        write_boundary="Writes content-read receipt JSON only under explicit output directory.",
        allows_artifact_writes=True,
        approval_mode=MODE_NONE,
        approval_boundary="None.",
        output_behavior="Outputs content-read receipts with content and redacted excerpt digests.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_ARTIFACT_ONLY,
        runtime_boundary="Composes config resolution, git snapshot, read/content receipts, context pack, routing recommendation, verification plan, golden path, and handoff; dry-run by default with no model/MCP/shell/commit authority.",
        ...
        notes="Founder/operator lane orchestrating existing governed surfaces without widening authority.",
        allows_artifact_writes=True,
    ),
    CommandAuthorityRecord(
