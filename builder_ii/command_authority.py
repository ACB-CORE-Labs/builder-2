...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_ARTIFACT_ONLY,
        runtime_boundary="Unified onboarding orchestrator composing the governed onboarding pipeline (setup plan, overlay plan, rollback snapshot, intent report); prompts only for the four wizard decisions, registry-validates every answer, and echoes the five documented-default decisions; no Goose, model, MCP, deepagents, patch, or shell execution.",
        ...
        output_behavior="Prints selected decisions, defaulted decisions with override flags, artifact paths, digests, and the deferred apply command to stdout.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_3,
        promotion_state=STATE_HITL_RUNTIME_CANDIDATE,
        runtime_boundary="Restores codebase to pre-apply state using the generated reverse patch, after a working-tree drift preflight.",
        ...
        output_behavior="Applies reverse diff; refuses if the working tree drifted since apply.",
        failure_mode="Refuses before touching the tree and writes a rollback-failure receipt carrying a recovery block (pre-apply HEAD, exact restore command with data-loss warning, chain-invalidation event) on drift or failed reverse apply; exits non-zero.",
        notes="B2 patch rollback governed path. Distinct rollback approval + drift-hardened preflight (plan item 1.4).",
        allows_source_writes=True,
    ),
    CommandAuthorityRecord(
