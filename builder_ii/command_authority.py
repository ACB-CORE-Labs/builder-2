...
COMMAND_AUTHORITY_REGISTRY: tuple[CommandAuthorityRecord, ...] = (
    ...
    *READONLY_TUI_AUTHORITY_RECORDS,
    CommandAuthorityRecord(
        ...
        tier=TIER_2,
        promotion_state=STATE_OPERATOR_MANAGED,
        runtime_boundary="Reads passive governance artifacts to render a spatial TUI; delegates execution to core tools. Refuses to launch unless invoked with the required --experimental flag; the surface itself is a display-only mockup (fake tier evaluation, fabricated chain digest, notify-only HITL approve/reject) pending post-beta wiring.",
        write_boundary="No direct write authority at TUI render level.",
        approval_mode=MODE_EXPLICIT_OPERATOR_INVOCATION,
        approval_boundary="Operator must explicitly launch TUI with --experimental.",
        output_behavior="Takes over terminal with Textual TUI interface.",
        failure_mode="Exits non-zero if --experimental is absent, textual is missing, or the UI crashes; target unchanged.",
        notes="STRATUM Command & Control TUI. Gated behind --experimental (plan D5 / item 3.13) — pre-release mockup, not yet wired to real governance state.",
    ),
    CommandAuthorityRecord(
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_ARTIFACT_ONLY,
        runtime_boundary="Binds human approval to the exact candidate digest; one typed prefix seals the whole envelope (lane policy, root budget, allowed obligation kinds, refused lanes) when the candidate declares one.",
    ...
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_SPEC_ONLY,
        runtime_boundary="PLANNED Ladder 4 PR-5 surface, NOT YET IMPLEMENTED: the invokable CLI lands in PR-5. Specified boundary — a deterministic read-only walk over a run's obligations, discharges, and ledger events rendering an OPEN / SATISFIED / UNVERIFIED / VIOLATED / BLOCKED board; no model, no execution, no writes.",
        write_boundary="No writes (specified). Not yet invokable.",
        approval_mode=MODE_NONE,
        approval_boundary="None.",
        output_behavior="Will print the obligation status board (PR-5).",
        failure_mode="Not yet invokable; specified to exit non-zero on a violated/broken obligation chain or missing run artifacts once implemented.",
        notes="Spec-only. Registered ahead of its CLI so PR-5 (a Sonnet new-file surface) never edits the contended command registry; running it today errors with no-such-command until PR-5 lands.",
    ),
    CommandAuthorityRecord(
        ...
        tier=TIER_1,
        promotion_state=STATE_SPEC_ONLY,
        runtime_boundary="PLANNED Ladder 4 PR-5 surface, NOT YET IMPLEMENTED: the invokable CLI lands in PR-5. Specified boundary — a deterministic read-only belief trace for one obligation (believed?, required evidence, what is attached, whether consumed); no model, no execution, no writes.",
        write_boundary="No writes (specified). Not yet invokable.",
        approval_mode=MODE_NONE,
        approval_boundary="None.",
        output_behavior="Will print a single-obligation belief/discharge trace (PR-5).",
        failure_mode="Not yet invokable; specified to exit non-zero on a violated/missing chain for the referenced obligation once implemented.",
        notes="Spec-only. Registered ahead of its CLI so PR-5 (a Sonnet new-file surface) never edits the contended command registry; a DISCHARGED_UNVERIFIED obligation will be reported as not believed once implemented.",
    ),
    CommandAuthorityRecord(
