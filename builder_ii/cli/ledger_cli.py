...
def audit(
    ...
    def action() -> dict[str, Any]:
        workflow_dirs = (
            [_resolve_output_dir(session_id, workflows_dir)] if session_id else _workflow_dirs(workflows_dir)
        )
...
def index_receipt(
    ...
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Optional ledger record output path under .builder/ledger."
    ),
) -> None:
...
def query_receipts(
    target_repo: Path = typer.Option(
        Path("."), "--target-repo", help="Target repository whose .builder/ledger directory should be read."
    ),
    ledger_root: Path | None = typer.Option(
        None,
        "--ledger-root",
        help="Explicit verification execution ledger root; defaults to --target-repo/.builder/ledger.",
    ),
    receipt_digest: str | None = typer.Option(
        None, "--receipt-digest", help="Filter by referenced verification execution receipt digest."
    ),
    chain_digest: str | None = typer.Option(
        None, "--chain-digest", help="Filter by verification execution chain digest."
    ),
