...
def delegate() -> None:
    ...
    console.print("builder-deepagents delegate is forbidden/unpromoted; use passive artifacts only.")
    raise typer.Exit(1)
...
def work_plan(
    ...
    assignment_plan: Path = typer.Option(..., "--assignment-plan", help="Path to agent assignment plan JSON"),
    assignment_dry_run: Path = typer.Option(..., "--assignment-dry-run", help="Path to orchestration dry-run JSON"),
    policy_path: Path = typer.Option(..., "--policy", help="Path to deepagents governed policy JSON"),
    readiness_path: Path = typer.Option(..., "--readiness", help="Path to deepagents dependency readiness JSON"),
    proposed_subagents: str | None = typer.Option(
        None, "--proposed-subagents", help="Comma-separated proposed subagents"
    ),
    expected_outputs: str | None = typer.Option(None, "--expected-outputs", help="Comma-separated expected outputs"),
    review_gates: str | None = typer.Option(None, "--review-gates", help="Comma-separated review gates"),
    blocked_capabilities: str | None = typer.Option(
        None, "--blocked-capabilities", help="Comma-separated blocked capabilities"
    ),
    output: Path | None = typer.Option(None, "--output", help="Write work plan JSON to path"),
) -> None:
...
def assign_subagent(
    ...
    subagent_profile: str = typer.Option(..., "--subagent-profile", help="Subagent profile name"),
    work_plan_path: Path = typer.Option(..., "--work-plan", help="Path to deepagents work plan JSON"),
...
def record_result(
    ...
    output: Path | None = typer.Option(None, "--output", help="Write subagent result JSON to path"),
) -> None:
...
def review_result_command(
    target: str = typer.Option("builder", "--target", help="Target profile: generic, builder, core"),
    disposition: str = typer.Option(..., "--disposition", help="accepted_as_proposal, needs_revision, or rejected"),
    subagent_result_path: Path = typer.Option(..., "--subagent-result", help="Path to subagent result JSON"),
    subagent_assignment_path: Path = typer.Option(
        ..., "--subagent-assignment", help="Path to subagent assignment JSON"
    ),
    output: Path | None = typer.Option(None, "--output", help="Write subagent review JSON to path"),
) -> None:
...
def request_human_gate(
    target: str = typer.Option("builder", "--target", help="Target profile: generic, builder, core"),
    reviewed_artifact_path: Path = typer.Option(..., "--reviewed-artifact", help="Path to reviewed artifact JSON"),
    output: Path | None = typer.Option(None, "--output", help="Write human gate request JSON to path"),
) -> None:
...
def record_blocked_action(
    target: str = typer.Option("builder", "--target", help="Target profile: generic, builder, core"),
    denied_capability: str = typer.Option(..., "--denied-capability", help="Blocked capability name"),
    triggering_artifact_path: Path | None = typer.Option(
        None, "--triggering-artifact", help="Optional triggering artifact JSON path"
    ),
    output: Path | None = typer.Option(None, "--output", help="Write blocked action record JSON to path"),
) -> None:
...
def proposal_result(
    target: str = typer.Option("builder", "--target", help="Target profile: generic, builder, core"),
    work_plan_path: Path = typer.Option(..., "--work-plan", help="Path to deepagents work plan JSON"),
    reviewed_result_path: list[Path] = typer.Option(..., "--reviewed-result", help="Repeatable reviewed result path"),
    output: Path | None = typer.Option(None, "--output", help="Write proposal result JSON to path"),
) -> None:
...
def execution_candidate(
    work_plan: Path = typer.Option(..., "--work-plan", help="Path to passive deepagents work plan JSON"),
    output_root: Path = typer.Option(..., "--output-root", help="Root directory allowed for promoted run artifacts"),
...
def backend_readiness(
    module_name: str = typer.Option("deepagents", "--module-name", help="Optional backend module to inspect"),
    package_name: str = typer.Option("deepagents", "--package-name", help="Optional backend package name"),
...
def replay_run(
    events_dir: Path = typer.Option(..., "--events-dir", help="Directory containing deepagents event records"),
    output: Path = typer.Option(..., "--output", help="Write replay report JSON to path"),
) -> None:
...
def evidence_bundle(
    ...
    event_ledger: Path = typer.Option(..., "--event-ledger", help="Event ledger JSON path"),
    replay_report: Path = typer.Option(..., "--replay-report", help="Replay report JSON path"),
    checkpoint: Path | None = typer.Option(None, "--checkpoint", help="Optional checkpoint JSON path"),
    output: Path = typer.Option(..., "--output", help="Write evidence bundle JSON to path"),
) -> None:
