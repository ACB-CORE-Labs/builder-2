...
def invoke(
    ...
    enforce_command_authority(
        "builder-tools invoke", requested_effects=("external_tool", "artifact_write", "state_write")
    )
    try:
    ...
    try:
        receipt = execute_tool_envelope(
            envelope=env_data, envelope_path=envelope, policy=pol_data, policy_path=policy_path
        )
