...
def atom(
    source_artifact: Path = typer.Argument(..., exists=True, dir_okay=False, readable=True, help="Validated source artifact to wrap as a governed memory atom."),
    output: Path | None = typer.Option(None, "--output", "-o", help="Optional output path for the memory atom JSON artifact."),
    ...
    claim_boundary: str | None = typer.Option(None, "--claim-boundary", help=f"Claim boundary override: {', '.join(CLAIM_BOUNDARIES)}"),
    ...
    source_truth_state: str = typer.Option("SOURCE_BOUND", "--source-truth-state", help=f"Source truth state: {', '.join(SOURCE_TRUTH_STATES)}"),
    summary_origin: str | None = typer.Option(None, "--summary-origin", help=f"Summary origin: {', '.join(SUMMARY_ORIGINS)}"),
    created_at: str | None = typer.Option(None, "--created-at", help="RFC3339 UTC timestamp for deterministic fixtures."),
) -> None:
...
def index(
    atom_paths: list[Path] = typer.Argument(..., exists=True, dir_okay=False, readable=True, help="Memory atom artifact paths to include in the index."),
    output: Path | None = typer.Option(None, "--output", "-o", help="Optional output path for the memory index artifact."),
    ...
    if len(target_profiles) != 1:
        ...
        raise typer.Exit(1)
    entries = [
        create_memory_index_entry(atom, path=path.resolve())
        for atom, path in zip(atoms, atom_paths, strict=True)
    ]
...
def search(
    index_path: Path = typer.Argument(..., exists=True, dir_okay=False, readable=True, help="Memory index artifact path."),
    query: str = typer.Option("", "--query", "-q", help="Deterministic lexical query. Empty query replays active atoms in stable order."),
    limit: int = typer.Option(10, "--limit", min=1, help="Maximum number of matches to emit."),
    output: Path | None = typer.Option(None, "--output", "-o", help="Optional output path for the memory search result artifact."),
    created_at: str | None = typer.Option(None, "--created-at", help="RFC3339 UTC timestamp for deterministic fixtures."),
) -> None:
...
def reconstruct(
    index_path: Path = typer.Argument(..., exists=True, dir_okay=False, readable=True, help="Memory index artifact path."),
    query: str = typer.Option("", "--query", "-q", help="Deterministic lexical query. Empty query replays active atoms in stable order."),
    max_atoms: int = typer.Option(5, "--max-atoms", min=1, help="Maximum number of selected atoms in the reconstruction."),
    output: Path | None = typer.Option(None, "--output", "-o", help="Optional output path for the memory reconstruction artifact."),
    created_at: str | None = typer.Option(None, "--created-at", help="RFC3339 UTC timestamp for deterministic fixtures."),
) -> None:
