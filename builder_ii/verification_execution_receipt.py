...
def _default_timeout_policy() -> dict[str, Any]:
    return {
        ...
        "timeout_seconds_source": "command_profile",
...
def _default_cwd_policy() -> dict[str, Any]:
    return {
        "cwd_source": "validated_target_repo",
        "cwd_must_exist": True,
...
def finalize_verification_execution_receipt(
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "shell_enabled": False,
        "subprocess_mode": SUBPROCESS_MODE_NOT_STARTED,
...
def validate_verification_execution_receipt_artifact(data: Any) -> list[str]:
    ...
    if data.get("runner_mode") != RUNNER_MODE_CONTRACT_ONLY:
        errors.append(f"runner_mode must be {RUNNER_MODE_CONTRACT_ONLY} for B1.3A")
    ...
    if data.get("subprocess_mode") != SUBPROCESS_MODE_NOT_STARTED:
    ...
    errors.extend(
        _validate_policy(
            ...
            ("ambient_environment_forwarded", "secrets_forwarded", "model_provider_keys_forwarded", "mcp_credentials_forwarded"),
        )
    )
