...
def render_platform_status(
    ...
    *,
    ...
    table.add_row(
        ...
        _label("compliance"),
        _value(f"literals={'OK' if comp.init_literals_ok else 'FAIL'}  refusal={'OK' if comp.refusal_probe_ok else 'FAIL'}"),
    )
...
def render_doctor(
    ...
    *,
    ...
    row("pass" if goose_ok else "warn", "goose", goose_status(), hint="run: pip install goose-ai" if not goose_ok else "")
    ...
    if b_ok:
        ...
        row("pass" if s_ok else "warn", "served model", s_msg, hint="run: builder switch-model <alias>" if not s_ok else "")
    ...
    # Active model
    active_status = next(
        (m for m in model_status_report(settings) if m.alias == settings.model_alias), None
    )
    if active_status:
    ...
    row("pass" if hf_bin else "warn", "hf cli", hf_bin or "not found", hint="pip install huggingface_hub[cli]" if not hf_bin else "")
...
def render_model_roster(
    ...
    *,
    ...
    for dfn in model_definitions(settings):
        ...
        alias_txt = (
            f"[{_C['active']} bold]{dfn.alias}[/]{_active_badge()}"
            if is_active
            else f"[{_C['bold']}]{dfn.alias}[/]"
        )
    ...
    *,
    ...
    console.print(
        Panel(
            ...
            + (
                f"\n\n  [{_C['hint']}]evidence: {evidence_path}[/]"
                if evidence_path and verbose
                else ""
            ),
...
def render_handoff_summary(
    ...
    *,
    ...
    next_steps = (
        (getattr(note, "next_steps", None) or note.get("next_steps", []))
        if hasattr(note, "get")
        else []
    )
...
def render_golden_path_summary(
    ...
    *,
    ...
    console.print(f"  {glyph} [{_C['bold']}]{target}[/]  [{_C['hint']}]{'all steps passed' if valid else 'steps failed'}[/]")
