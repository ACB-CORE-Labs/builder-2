...
**Version:** 1.1 (adversarially reviewed; external review folded)
**Date:** 2026-07-07 (v1.1 same day: folded Gemini 3.1 Pro external review + operator craft doctrine)
...
| D7 | Sandbox model for pytest verification lane | **RECOMMENDED, not yet ratified:** env-allowlist subprocess + **schema-enforced** execution-risk acknowledgment field in the approval artifact, required **only** for target-code-executing profiles (`pytest_full`; not `platform_status`/`docs_audit`, which run builder-II's own fixed argv — verified `verification_execution_runner.py:69-84`). Runner refuses approvals missing the ack. Beta scope pinned to **trusted local repositories only** (FIRST_SESSION + beta charter + known-limitations). Container isolation post-beta. | pending |
...
1. **Target-repo test execution is real code execution** — D7 is the most safety-significant open
   decision; it gates 1.2/1.3. The bounded runner constrains *invocation*, never *code behavior*;
   beta scope = trusted local repos only, and the realistic beta-window threat is transitive (a
   dependency's `conftest`/plugin), which the risk-ack language names explicitly.
