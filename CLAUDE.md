...
CodeVault is a separate commercial plugin. Open core contains no CodeVault implementation or
design records; `builder_ii/cli/code_vault_cli.py` is only an optional import boundary with an
explicit non-zero refusal when the plugin is unavailable.
