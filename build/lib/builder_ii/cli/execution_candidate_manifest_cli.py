...
def candidate_manifest(
    approval_boundary_path: Path = typer.Option(..., "--approval-boundary-path", help="Path to approval boundary JSON"),
    decision_path: Path = typer.Option(..., "--decision-path", help="Path to promotion decision JSON"),
    review_path: Path = typer.Option(..., "--review-path", help="Path to promotion review JSON"),
    request_path: Path = typer.Option(..., "--request-path", help="Path to promotion request JSON"),
    source_proposal_path: list[Path] = typer.Option(
        ..., "--source-proposal-path", help="Path(s) to source proposal JSONs"
    ),
    target_profile_path: Path = typer.Option(..., "--target-profile-path", help="Path to target profile JSON"),
    command_authority_path: Path = typer.Option(..., "--command-authority-path", help="Path to command authority JSON"),
    verification_profile_path: Path = typer.Option(
        ..., "--verification-profile-path", help="Path to verification profile JSON"
    ),
    output: Path = typer.Option(..., "--output", help="Output path for the candidate manifest JSON"),
    command_preview: list[str] = typer.Option(None, "--command-preview", help="Inert command previews to include"),
    no_mutation_assertion: bool = typer.Option(False, "--no-mutation-assertion", help="Assert that no mutation occurs"),
    rollback_plan_path: Path | None = typer.Option(
        None, "--rollback-plan-path", help="Optional path to rollback plan JSON"
    ),
    git_state_path: Path | None = typer.Option(None, "--git-state-path", help="Optional path to git state JSON"),
    preflight_path: Path | None = typer.Option(None, "--preflight-path", help="Optional path to preflight JSON"),
    ...
    boundary_ref = _create_ref(boundary, path=approval_boundary_path, role="approval_boundary")
    ...
    cmd_auth_ref = _create_ref(cmd_auth, path=command_authority_path, role="command_authority")
    ver_prof_ref = _create_ref(ver_prof, path=verification_profile_path, role="verification_profile")

    rollback_plan_ref = None
    if rollback_plan_path is not None:
        rb_data = _load_json(rollback_plan_path)
        rollback_plan_ref = _create_ref(rb_data, path=rollback_plan_path, role="rollback_plan")
    ...
    if specialized_candidate_path is not None:
        ...
        spec_ref = _create_ref(spec_data, path=specialized_candidate_path, role="specialized_candidate")
    ...
    source_approval_boundary_decision_record_state = boundary.get("source_decision_record_state")
...
def validate_candidate_manifest(
    paths: list[Path] = typer.Argument(..., help="Paths to candidate manifest artifact JSON files"),
) -> None:
    ...
    for path in paths:
        ...
        if kind == EXECUTION_CANDIDATE_MANIFEST_KIND:
            ...
            errs = [f"Unknown or unsupported candidate manifest artifact kind: {kind} in {path}"]
