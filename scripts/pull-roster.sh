...
# Compatible with the default macOS /bin/bash 3.2: no associative arrays.
#
# Usage:
#   bash scripts/pull-roster.sh status
#   bash scripts/pull-roster.sh recommended      # phi-reasoning + qwen-coder
#   bash scripts/pull-roster.sh fast             # phi-reasoning only
#   bash scripts/pull-roster.sh primary          # qwen-coder only
#   bash scripts/pull-roster.sh all-safe         # recommended + Gemma/Llama alternates
#   bash scripts/pull-roster.sh candidates       # CodeGeeX/Qwen14/Qwen3/DeepSeek candidates
...
repo_for_alias() {
  case "$1" in
    phi-reasoning) echo "${CORE_AGENT_MLX_MODEL_PHI:-mlx-community/Phi-4-mini-reasoning-4bit}" ;;
    qwen-coder) echo "${CORE_AGENT_MLX_MODEL_QWEN:-mlx-community/Qwen2.5-Coder-7B-Instruct-4bit}" ;;
    gemma-fast) echo "${CORE_AGENT_MLX_MODEL_FAST:-mlx-community/gemma-4-e4b-it-4bit}" ;;
    gemma-primary) echo "${CORE_AGENT_MLX_MODEL_PRIMARY:-mlx-community/gemma-4-12B-it-4bit}" ;;
    llama) echo "${CORE_AGENT_MLX_MODEL_LLAMA:-mlx-community/Meta-Llama-3.1-8B-Instruct-4bit}" ;;
    codegeex) echo "${CORE_AGENT_MLX_MODEL_CODEGEEX:-mlx-community/codegeex4-all-9b-4bit}" ;;
    qwen-coder-14b) echo "${CORE_AGENT_MLX_MODEL_QWEN14:-mlx-community/Qwen2.5-Coder-14B-Instruct-4bit}" ;;
    qwen3-coder-heavy) echo "${CORE_AGENT_MLX_MODEL_QWEN3_CODER:-mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit}" ;;
    deepseek) echo "${CORE_AGENT_MLX_MODEL_DEEPSEEK:-mlx-community/DeepSeek-Coder-V2-Lite-Instruct-4bit}" ;;
    *) return 1 ;;
  esac
}
...
progress() {
  ...
  if ! repo="$(repo_for_alias "$alias")"; then
    ...
    echo "Known aliases: $(known_aliases)" >&2
    exit 1
  fi
  local dir
  dir="$(cache_slug "$repo")"
...
case "$cmd" in
  ...
  candidates)
    ...
    ;;
  alias)
    pull_alias "${2:?usage: bash scripts/pull-roster.sh alias <model-alias>}"
    ;;
  *)
    echo "Unknown command: $cmd" >&2
    echo "Usage: bash scripts/pull-roster.sh status|recommended|fast|primary|all-safe|candidates|alias <name>" >&2
    exit 1
    ;;
esac
