...
| TUI exploration driver | `smoke_only` / `validation_only` | Tier 1 (`scripts/tui_driver.py`, `scripts/validate_tui_exploration.py`) | Spawns/tests read-only interactive TUIs via pexpect; no write or mutation authority. | `tui-exploration-report.json` | `python scripts/tui_driver.py --smoke`, `pytest tests/scenarios/tui_exploration.py` | Operator reviews snapshot reports and verifies TUI integrity. | Exits non-zero on timeout or error; target repository remains untouched. |
