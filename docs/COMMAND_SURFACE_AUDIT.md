...
- `builder-code-vault structural-field TARGET --output PATH [--repo-map PATH | --repo-path PATH]` builds, validates, and writes a governed `builder_ii.code_vault.structural_field` artifact for the `signature` fact kind: an arity/shape descriptor (parameter counts, never names or text) for top-level functions, top-level async functions, and methods. Facts bind to `subject_layout_id` (top-level defs reproduce the F0 frame's `layout_id` scheme exactly; methods extend it — the field is a superset of the frame, never a subset). Non-Python files, syntax-error files, and unreadable/non-UTF-8 files in scope become sorted `unsupported[]` residue entries, never a fabricated fact.
