...
**Current capability (truth):** `artifact_only` / `validation_only` / `recommendation_only` + S2/P4 **HITL candidates** — **not** S3 live-enabled multi-agent. P6 backends are **opt-in substrate**, not S4 promoted.

**Progress marker (living cursor):** [`docs/WRP_MASTERY_PROGRESS.md`](WRP_MASTERY_PROGRESS.md).
...
[x] S3 readiness + HUMAN decision=blocked
[x] P6 opt-in backends (substrate)  [x] P7 W5 repo-state + ceremony template
...
| `OPEN` | Required for mastery; not done (or deliberately deferred research) |
| `STAGE` | Unlocked by promotion stage S1–S4 |
| `DEFERRED` | Named research track; not accidental omission |
...
| Dual-platform exchange + G0–W5 Governor cert | `LANDED` | P7 ops | `artifacts/wrp_exchange/`; template `docs/templates/WRP_AUTHORITY_PR_CEREMONY.md` |
...
| Promotion **decision** S1–S3 | `PARTIAL` | P1 | **S1+S2 approved**; S3 readiness G-LEAD PASS + HUMAN **blocked** (`planning/evidence/wrp_s3_{readiness,decision}.json`) — not enabled |
...
| EmbeddingBackend + kNN (hash default; ModernBERT-class opt-in) | `LANDED` | P2.0 / P6 | `resolve_embedder`; `OptionalModernBertBackend` fail-closed; S4 embedder promo open |
...
| OPA export + optional OPA eval parity | `LANDED` | P2.3 / P6 | `opa_adapter.py` + pure-MSDA parity tests + `builder-wrp opa-eval` |
| Classifier ↔ EmbeddingBackend wire | `LANDED` | P2.0 | `use_embedding` / `BUILDER_II_WRP_EMBED`; HashingEmbedder+kNN; default metric path |
...
| SubtaskGraph plan + digest replay | `LANDED` | P2.6 / P2.8 / P7 | graph runtime + W5 `commit_id`/`tree_hash` binding (`repo_state.py`, `replay_graph_digests`) |
...
| vLLM WRP research profile | `LANDED` | P6 | `vllm_profile.py` + `docs/plan/WRP_VLLM_RESEARCH_PROFILE.md` + `builder-wrp vllm-profile` (stub; not default) |
| LangGraph optional adapter | `LANDED` | P2.6 / P6 | `langgraph_adapter.py` pure projection + opt-in compile; skip-if-missing tests |
| Trained R head research track | `DEFERRED` | P6 | `docs/plan/WRP_R_HEAD_RESEARCH_TRACK.md` — offline/no silent override; not production |
| W5 Gitea/repo state reconstructive match | `LANDED` | P2.8 / P7 | `repo_state.capture_repo_state` + replay `repo_state_match`; CLI `--bind-repo` / commit-tree flags |
| Merge ceremony on every authority PR | `PARTIAL` | P7 | process + `docs/templates/WRP_AUTHORITY_PR_CEREMONY.md`; practiced on S2/P5; uneven archival (H4) |
| Scoped `enabled` live path | `PARTIAL` | S3 (after P5) | Readiness G-LEAD **PASS-MERGE** (#146) + HUMAN **blocked** (#147); re-open only with new evidence/design — no enablement code |
| Cloud provider model invoke | `OPEN` | S2+/S4 | record/stub only (H6); not this P6 landing |
| S4 backend promotion decisions | `OPEN` | S4 | P6 substrate only; each backend needs own readiness/decision |
...
```text
[~] Master-Plan W0–W5 green with Maker + Governor evidence   (W5 code LANDED; full dual-platform green PARTIAL)
...
[~] Dual-platform ceremony for authority changes             (template + S2/S2-v2/P5; P4 cert thin)
[~] CAPABILITY_PROMOTION / matrix / command_authority match  (WRP updated; watch drift on each PR)
[x] Heavy backends exist as opt-in with tests; defaults M1-safe (P6 LANDED; S4 promo OPEN)
[~] This gap matrix has zero OPEN rows remaining             (intentional OPEN: S4, cloud invoke; DEFERRED: R head)
```
