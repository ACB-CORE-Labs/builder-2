...
| **B** | Headless streamed governed run | ~`adapters/goose/goose_runtime_harness.py` (`run_governed_streaming`, `request_stop`, `goose run` capability detection) ~`cli/goose_cli.py` (`run-governed`) ~`governance/ledger/event_ledger.py` (run lifecycle event types) ~`governance/authority/authority_registry.py` | `goose_run_started`/`goose_run_completed`/`run_stop_requested` `event_record`s; reuses launch/close/postflight receipts | `tests/test_goose_run_governed.py`: a real fake-goose executable streams to a byte-capped log; lifecycle chains and replays; child exit code propagates; a mutating run fails the postflight; argv built only from advertised flags; SIGTERM records intent and TERM→KILL escalates | inside |
