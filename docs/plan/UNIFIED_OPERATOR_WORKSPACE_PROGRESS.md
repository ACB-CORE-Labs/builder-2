...
| UOW-106 | 1 | `FOCUSED_VERIFIED` | canonical governed Goose start/close evidence and events | five rejected exact tips and their lesions are retained below; cumulative correctness review passed at `813aced3c3f665d066fed3e022773ce0c374381e`; UOW-105 lifecycle convergence is focused-verified |
...
| 2026-08-27 | Deep Agents exact event raw reconstruction and terminal state cross-binding | `uv run pytest -q tests/test_run_lifecycle_scenarios.py tests/test_run_view.py` | PASS | resolved the remaining review 3 and review 4 blockers: full event graph dereferencing, raw byte replay reconstruction, terminal state coherence validation (RUNNING != COMPLETED), and malformed evidence fail-closed error boundaries |
| 2026-08-27 | exact-tip local CI receipt for review 4 corrections | `bash scripts/ci.sh --receipt .builder/dev-evidence/unified-operator-ps1/gate-battery-uow105-r5.json` on `06d07a081b96bbd87ad2c89fcb5fb78ecd0e6138` (base `afaac2af3ba8ee93fd4b45ae81306fd88080831d`) | PASS | 11/11 blocking gates, 0 gate skips; pytest 3079 passed, 1 skipped, 0 failed; receipt digest `f59cb40b`; working tree clean |
...
| F-014 | Path concatenation plus ordinary `mkdir`/`write_text` allowed symlinked session components to redirect custody | custody must validate its own session id and use component-wise no-follow directory admission plus exclusive file creation | implemented and covered by the sixth cumulative audit PASS |
| F-015 | Per-artifact event checks did not reconstruct close-to-launch/postflight/transcript bindings | one reusable reconstruction validator must serve persistence and RunView | implemented and covered by the sixth cumulative audit PASS |
| F-016 | Postflight `valid` only tracked the caller-supplied unexplained list | validators must prove an exact disjoint partition of detected mutations and mode/evidence consistency | implemented and covered by the sixth cumulative audit PASS |
| F-017 | Runtime events allocated sequence and filename outside a lock | lock allocation, prior-chain validation, and exclusive canonical JSON creation as one serialized operation | implemented with deterministic same/different-type concurrency lesions and covered by the sixth cumulative audit PASS |
| F-018 | A dangling `events.wal` symlink bypassed an `exists()` guard, while valid-but-divergent WAL and JSON inventories could project different histories | designate canonical JSON as the sole event authority; treat any retained WAL as a reconciliation-only legacy mirror and fail closed on symlink or byte-semantic divergence | implemented; runtime append refuses retained WAL until reconciled and retired; covered by the sixth cumulative audit PASS |
| F-019 | Transcript export validated a temporary pathname and later gave that mutable pathname to Goose | preserve both directory and file descriptor custody across child export, then install by descriptor-bound inode only if the canonical directory identity is unchanged | implemented with export-failure and directory-inode-swap lesions; covered by the sixth cumulative audit PASS |
| F-020 | Approved mutation mode accepted a non-empty opaque object as authority evidence | require the mode-specific exact reference set, session/target identity, SHA-256 syntax, and reconstruction of every referenced byte before close can validate | implemented and covered by the sixth cumulative audit PASS |
| F-021 | Retaining a transcript file descriptor did not prove that the temporary filename still named that inode at installation | open the name relative to the retained directory descriptor with no-follow semantics and require exact device/inode equality before linking | implemented with a temporary-name replacement lesion; covered by the sixth cumulative audit PASS |
| F-022 | Canonical close reconstruction checked approved-reference names and digests but did not repeat the runtime's namespace, owning-validator, cross-binding, scope, and final-state checks | use the same approved patch/rollback reconstruction functions before persistence and during later RunView custody validation | implemented with digest-correct foreign-file refusal; covered by the sixth cumulative audit PASS |
| F-023 | A temporary-name swap inside the hard-link boundary could occur after the pre-link inode comparison | reopen the canonical destination after linking, compare it to the retained export inode, and remove/refuse any mismatch | implemented with a link-boundary swap lesion; covered by the sixth cumulative audit PASS |
| F-024 | Shared runtime reconstruction helpers assumed already-screened JSON object shapes and could raise through persistence or RunView | make the canonical reconstruction seam total: unexpected evidence types or validator exceptions become explicit fail-closed custody errors | implemented with malformed-evidence and rollback integration coverage; covered by the sixth cumulative audit PASS |
| F-025 | Post-link validation compared only an already-open destination fd, not the canonical directory entry at acceptance | add a final no-follow, dirfd-relative stat of the canonical name and remove/refuse any inode mismatch | implemented with a post-open canonical-name replacement lesion; covered by the sixth cumulative audit PASS |
...
| --- | --- | --- |
| R-001 | Base reconciliation after predecessor review | PR #21 merged into `main` (`afaac2a`) and PR #22 closed; branch PR #23 is based directly on canonical `main` (`afaac2af3ba8ee93fd4b45ae81306fd88080831d`) |
