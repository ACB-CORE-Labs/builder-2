...
| UOW-106 | 1 | `FOCUSED_VERIFIED` | canonical governed Goose start/close evidence and events | first independent audit blocked the initial tip; repaired symlink, reconstruction, mutation-partition, concurrency, and export-retry lesions now pass the focused battery; independent re-audit and full local CI remain required |
...
| F-017 | Runtime events allocated sequence and filename outside a lock | lock allocation, prior-chain validation, WAL append, and exclusive JSON mirror creation as one serialized operation | repair implemented locally with deterministic same/different-type concurrency lesions; independent re-audit pending |
