...
| **B** | CodeVault | Commercial-plugin availability | Open plugin help |
...
CodeVault is a separate, privately licensed plugin. The open-core `builder-code-vault` command is
an opaque optional-import seam: it delegates when the package is installed and otherwise prints an
upgrade notice and exits non-zero. Plugin implementation and command details are intentionally not
...
6. HITL only when changing authority — compose from STRATUM, approve in CLI
7. `audit-docs` before you write README claims
