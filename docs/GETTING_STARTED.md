...
| **G** | Goose hand-off | Auto-preps scaffold + read_only manifest if missing; then `start-readonly` | Fail-closed on authority; still no raw Goose |
...
5. **Runtime lane** — **G** auto-preps a passive read_only manifest if needed, then hands off to `start-readonly`  
...
# STRATUM: W shows recipe inventory; G auto-preps a read_only manifest if needed
# then hands off to builder-goose start-readonly (still authority-gated)
uv run builder stratum --experimental
# Optional: mint your own instead of the auto file
...
**Mental model:** recipes = *how the operator runtime should behave if launched*; manifests + promotion = *whether it may launch*. Auto-prep only creates a passive artifact — it does not start Goose or grant authority.
