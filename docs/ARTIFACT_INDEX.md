...
- `builder_ii.hitl_decision_envelope`
...
- `builder_ii.ratification_policy`
- `builder_ii.ratification_approval`
...
| `builder_ii.ratification_policy` | Per-point ratification level, tighten-only. The effective level is max(registry baseline, declared), so a declared level below the baseline is ignored rather than honoured | current |
| `builder_ii.ratification_approval` | Human decision bound to one ratification point and one exact subject digest, minted only by a typed digest-prefix confirmation. Re-verified against subject and clock at use time; replayable within its TTL against that same subject | current |
