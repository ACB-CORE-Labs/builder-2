...
| Pillar 2 — Cl(4,1) CGA geometric coherence | **Excised.** CodeVault moved to the commercial `builder-ii-code-vault` plugin; open core retains only the fail-closed CLI seam (`builder_ii/cli/code_vault_cli.py`). Cannot be audited here, and it would be confabulation to score it. |
