...
| HITL-approved verification execution | `PASSIVE_FOUNDATION` | B1.2/B1.3 |
...
B1.1 adds `builder_ii.verification_execution_plan` plus `builder-verify plan` and `builder-verify validate-plan` as a passive verification execution planning surface. The artifact is digest-stable and ARTIFACT_ONLY/PASSIVE_FOUNDATION; it does not run tests, execute shell/subprocess, call models/tools, invoke MCP, start Goose/deepagents, apply patches, mutate git, or promote actual verification execution. B1 is not complete; HITL approval and execution remain later B1 slices.
