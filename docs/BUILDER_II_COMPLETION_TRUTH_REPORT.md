...
| interactive setup wizard | NOT_STARTED | README has manual `.env` plus governed `builder-setup` commands; command: none | No wizard plans target repo, artifact root, profile choices, model/backend, Goose writes, recipes, skills, and disabled/passive/operational capabilities before apply. | R1 |
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/cli.py` setup/doctor/status, `builder_ii/goose_setup.py`, `builder_ii/setup_plan.py`, `builder_ii/setup_overlay.py`, `builder_ii/setup_rollback.py`, `builder_ii/setup_cli.py`, `tests/test_goose_setup.py`, `tests/test_setup_plan.py`, `tests/test_setup_overlay.py`, `tests/test_setup_rollback.py`, `tests/test_config_setup_cli.py`; commands: legacy `builder setup` redirect, `builder-setup plan/validate-plan/overlay-plan/validate-overlay-plan/rollback-snapshot/validate-rollback-snapshot/apply/validate-receipt/rollback/validate-rollback-receipt` | R1.4 reconciles the legacy setup surface: `builder setup` now fails closed and redirects to the governed artifact chain. Apply/rollback are digest-bound and non-runtime. Interactive onboarding UX and runtime promotion remain missing. | R1 |
