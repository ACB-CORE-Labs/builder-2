...
Historical report retained for context; it is not live sequencing truth. The current capability matrix reports 19 `OPERATIONALLY_VERIFIED` capabilities. Open-source-v1 sequencing is governed by `docs/plan/OPEN_SOURCE_V1_COMPLETION_PLAN.md` and its per-plan-set evidence records.
...
|---|---|---|---|---|
| generic platform identity | PASSIVE_FOUNDATION | `README.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, `docs/adrs/ADR-0003-builder-ii-generic-platform-identity-and-capability-factory.md`, `tests/test_operator_command_surface.py`, `tests/test_command_authority.py`; commands: none dedicated | Identity is documented and tested, but truth-state enforcement is not machine-derived. Legacy root commands still use CORE naming internally in places such as `builder_ii/adapters/goose/goose_setup.py`. | R0 |
| target profiles | PASSIVE_FOUNDATION | `builder_ii/lifecycle/setup/target_profiles.py`, `builder_ii/targets_cli.py`, `tests/test_target_profiles.py`; commands: `builder-targets list/show/validate/artifact/demo/readonly-founder-demo` | Profiles are no-runtime artifacts. Need target registration lifecycle with authority state and target-root policy for operational runtime. | B3 |
| agent profiles | PASSIVE_FOUNDATION | `builder_ii/routing/agent_profiles.py`, `builder_ii/agent_cli.py`, `tests/test_agent_profiles.py`; command: `builder-agent validate` | Roles record read/plan/proposal authority only. No runtime agent construction, approval, or receipt path. | B5 |
| verification profiles | PASSIVE_FOUNDATION | `builder_ii/lifecycle/candidate/verification_profiles.py`, `builder_ii/verification_cli.py`, `builder_ii/lifecycle/candidate/verification_profile_reports.py`, `tests/test_verification_profiles.py`, `tests/test_verification_profile_reports.py`; commands: `builder-verification list/show/artifact/validate` | Profiles propose checks and reject completed evidence claims. Missing HITL-approved runner and receipt binding. | B1 |
| context packs | PASSIVE_FOUNDATION | `builder_ii/core/repo_map.py`, `builder_ii/core/context_packs.py`, `builder_ii/session_cli.py`, `tests/test_repo_map.py`, `tests/test_governed_prepare_package.py`; commands: `builder-session repo-map/context-pack/prepare-package` | Canonical path is artifact-only. Legacy `builder-context` uses external scanning and git subprocesses outside governed read authority. Need B3 read authority policy. | B3 |
| profile packs | PASSIVE_FOUNDATION | `builder_ii/lifecycle/setup/profile_pack_manifest.py`, `profile_pack_render_plan.py`, `profile_pack_dry_run.py`, `profile_pack.py`, `tests/test_profile_pack.py`, `tests/test_profile_pack_cli.py`; commands: `builder-profile-pack scaffold/render/dry-run/validate` | Lifecycle is passive (`PLANNED_ONLY`, `RENDERED_ONLY`, `DRY_RUN_ONLY`, `PACKED_ONLY`). No runtime materialization. | R0 then defer runtime materialization |
| config schema | PASSIVE_FOUNDATION | `builder_ii/core/config_schema.py`, `builder_ii/core/config.py`, `docs/CONFIG_ONBOARDING.md`, `tests/test_config_schema.py`, `tests/test_config_setup_cli.py`; command: `builder-config schema/validate` | R1.1 adds a generic-first schema with disabled capability defaults and legacy `CORE_*` alias metadata. Digest-bound `builder-setup apply`/`rollback` exist for declared setup paths; ambient runtime authority and migration tooling remain unpromoted. | R1 |
| config source precedence | PASSIVE_FOUNDATION | `builder_ii/core/config_sources.py`, `builder_ii/config_cli.py`, `docs/CONFIG_ONBOARDING.md`, `tests/test_config_sources.py`, `tests/test_config_setup_cli.py`; command: `builder-config resolve/validate` | R1.1 records CLI/env/.env/config-file/profile/built-in precedence with source refs, redaction, warnings, errors, path policy, and deterministic digest. It remains artifact-only. | R1 |
| interactive setup wizard | PASSIVE_FOUNDATION | `builder_ii/lifecycle/setup/setup_onboarding.py`, `builder_ii/lifecycle/setup/onboarding_intent.py`, `builder_ii/setup_cli.py`, `builder_ii/cli.py`, `tests/test_onboarding_intent.py`, `tests/test_setup_onboarding_wizard_cli.py`; commands: `builder-setup wizard`, `builder onboarding` | R1.5 provides an interactive guided setup wizard emitting passive onboarding intent and plan artifacts. Onboarding commands print deferred apply commands only; setup mutation remains exclusively owned by `builder-setup apply`. Target/agent/verification profile editing wizards remain separate unbuilt slices. | R1 |
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/lifecycle/setup/setup_apply.py`, `builder_ii/lifecycle/setup/setup_rollback_execute.py`, `builder_ii/cli/setup_cli.py`; commands: `builder-setup apply`, `builder-setup rollback`, plus passive plan/overlay/rollback-snapshot chain | Digest-bound setup apply/rollback commands are STATE_ENABLED and legacy `builder setup` fails closed, but the capability row is not operationally verified: that flip requires an R1 closure audit (R1_OPERATOR_FLIPPED_CAPABILITIES), not command enablement. Passive plan/overlay validate chain remains passive scope. | R1 |
| Goose config overlay/rollback | PASSIVE_FOUNDATION | `builder_ii/lifecycle/setup/setup_overlay.py`, `builder_ii/lifecycle/setup/setup_rollback.py`, legacy `builder_ii/adapters/goose/goose_setup.py`, `tests/test_setup_overlay.py`, `tests/test_setup_rollback.py`, `tests/test_goose_setup.py`, `tests/scenarios/test_config_to_goose_projection_flow.py` | R1.2 describes Goose config overlay keys, recipe registration, secrets-preservation policy, prior-state markers, and future rollback operation metadata. No Goose config writes, apply, rollback execution, approved write boundary, or audit receipt exists. | R1 |
| recipe generator/wizard | ARTIFACT_ONLY | `recipes/`, `builder_ii/adapters/goose/goose_recipe_context_projection.py`, `tests/test_goose_recipe_context_projection.py`, `tests/test_session_wiring.py`; command surface through setup/projection paths | Recipe assets and projections exist. No generator/wizard, operator preview, apply receipt, rollback path, or target/profile compatibility check. | R1 |
| skill generator/installer/validator | MERGED_BUT_NOT_OPERATIONAL | `.agents/skills/`, `builder_ii/lifecycle/setup/setup_overlay.py`, `builder_ii/adapters/goose/goose_setup.py`, `tests/test_setup_overlay.py`, `tests/test_goose_setup.py`; commands: `builder-setup overlay-plan`, legacy `builder setup` redirect | R1.2 adds passive install-plan entries with source/destination digests and conflict notes. R1.4 disables legacy skill copying from `builder setup`. Operational skill install/copy, setup receipt promotion, rollback execution, and target-scoped approval are still missing. | R1 |
| target profile wizard | NOT_STARTED | `builder_ii/lifecycle/setup/target_profiles.py`, `builder_ii/targets_cli.py`, `tests/test_target_profiles.py`; command surface validates existing profiles | No guided target profile creation/editing, dry-run preview, source precedence binding, or setup receipt. | R1 |
| agent profile wizard | NOT_STARTED | `builder_ii/routing/agent_profiles.py`, `builder_ii/agent_cli.py`, `tests/test_agent_profiles.py`; command surface validates existing profiles | No guided agent profile creation/editing with authority preview and disabled runtime defaults. | R1 |
| verification profile wizard | NOT_STARTED | `builder_ii/lifecycle/candidate/verification_profiles.py`, `builder_ii/verification_cli.py`, `tests/test_verification_profiles.py`; command surface validates existing profiles | No guided verification profile creation/editing, command allowlist preview, target compatibility check, or no-execution proof. | R1 |
| deepagents/researcher setup wizard | NOT_STARTED | `builder_ii/adapters/deepagents/deepagents_policy.py`, `builder_ii/adapters/deepagents/deepagents_readiness.py`, `builder_ii/adapters/deepagents/deepagents_bridge_readiness.py`, tests around readiness/policy; commands: `builder-deepagents policy/readiness/validate*` | Optional dependency readiness exists, but no setup wizard for researcher/deepagents capability selection, denied defaults, receipts, or no-runtime proof. | R1 |
| setup receipt + rollback artifact | PASSIVE_FOUNDATION | `builder_ii/lifecycle/setup/setup_apply.py`, `builder_ii/lifecycle/setup/setup_receipt.py`, `builder_ii/lifecycle/setup/setup_rollback_execute.py`, `builder_ii/lifecycle/setup/setup_rollback_receipt.py`; commands: `builder-setup apply`, `builder-setup rollback`, `builder-setup rollback-snapshot` | R1.3A/B add digest-bound setup apply receipts and setup rollback execution for declared paths; the row stays passive-foundation pending an R1 closure audit. Ledger event and full replay binding remain partial. | R1 |
| model registry | OPERATIONALLY_VERIFIED | `builder_ii/routing/model_client_registry.py`, `builder_ii/routing/model_execution_gateway.py`; commands: `builder-model-policy`, `builder-model call` | Governed registry under execution gateway with policy bounds. Legacy `builder ask/start` remain operator-managed helpers outside the canonical lane. | B7 |
| model routing | OPERATIONALLY_VERIFIED | `builder_ii/routing/model_routing_policy.py`, `builder_ii/model_policy_cli.py`, `builder_ii/routing/model_execution_gateway.py`; commands: `builder-model-policy`, `builder-model call` | Routing recommendations consumed by governed gateway; cloud/external calls require explicit policy/approval. | B7 |
| model/provider execution | OPERATIONALLY_VERIFIED | `builder_ii/routing/model_execution_gateway.py`, `builder_ii/cli/model_cli.py`; commands: `builder-model call`, `builder-model standalone-call` (test/offline only) | Gateway enforces policy, registry, token caps, secret scan, envelope/receipt; `builder-model call` is ledger-bound operational lane. `standalone-call` is not ledger-bound proof. | B7 |
| tool registry | PASSIVE_FOUNDATION | `builder_ii/core/tool_registry.py`, `builder_ii/tools_cli.py`; commands: `builder-tools list/check/missing` | Registry and version probes exist; no invocation envelope or effect classification. `check` uses subprocess version probes as operator-managed tooling. | B7 |
| MCP/tool invocation | PASSIVE_FOUNDATION | `builder_ii/core/mcp_policy.py`, `builder_ii/core/tool_invocation_gateway.py`; commands: `builder-mcp inventory/policy/call`, `builder-tools invoke` | MCP inventory/policy/call surfaces exist; live MCP server execution remains unpromoted/stub-limited. Low-risk deterministic tool stubs are `OPERATIONALLY_VERIFIED` via `low-risk tool invocation` row. | B7 |
| passive orchestration assignment | PASSIVE_FOUNDATION | `builder_ii/core/orchestration_assignment.py`, `builder_ii/orchestration_cli.py`, `tests/test_orchestration_assignment.py`; commands: `builder-orchestration plan/render-assignment/dry-run/validate` | Assignment binds artifacts by digest but starts no agents. Runtime assignment execution must wait for B1/B5. | B5 |
| workflow/event ledger | PASSIVE_FOUNDATION | `builder_ii/core/workflow_orchestrator.py`, `workflow_records.py`, `event_ledger.py`, `workflow_cli.py`, `ledger_cli.py`, `tests/test_workflow_ledger.py`; commands: `builder-workflow *`, `builder-ledger *` | Ledger records passive workflow events only. Missing runtime event kinds for actual reads, execution, model/tool calls, rollback, memory mutation. | B1 then B6/B7/B8 |
| replay/audit | PASSIVE_FOUNDATION | `builder_ii/governance/ledger/event_ledger.py`, `builder_ii/core/artifact_chain_verification.py`, `tests/test_workflow_ledger.py`, `tests/test_artifact_chain_verification.py`; commands: `builder-ledger replay/audit/export`, `builder-chain verify` | Replay validates passive event order and artifact links, not operational side effects. Need replay policy for non-deterministic execution receipts. | B1 |
| readonly founder demo | PASSIVE_FOUNDATION | `builder_ii/core/readonly_founder_demo.py`, `builder_ii/targets_cli.py`, `tests/test_readonly_demo.py`, `tests/test_readonly_demo_idempotence.py`; command: `builder-targets readonly-founder-demo` | Demo writes passive inspection, patch proposal, verification plan, events, status. It does not inspect live content beyond artifacts or run verification. | defer after R0 |
| orchestration founder demo wrapper | PASSIVE_FOUNDATION | `builder_ii/core/readonly_founder_demo.py`, `docs/demos/CORE_READONLY_FOUNDER_DEMO.md`; command: `builder-targets readonly-founder-demo` | Wrapper is a passive workflow/event demonstration. Missing operator golden path that runs real governed read/verify loops. | B9 |
| HITL promotion bridge | PASSIVE_FOUNDATION | `builder_ii/governance/hitl/hitl_promotion_artifacts.py`, `builder_ii/hitl_promotion_cli.py`, `tests/test_hitl_promotion_artifacts.py`; commands: `builder-hitl promotion-request/review/decision/approval-boundary/rejection-record/validate-promotion` | Approval boundary is for candidate design only and requires a separate execution candidate. No execution authority. | B1 |
| execution candidate manifests | PASSIVE_FOUNDATION | `builder_ii/lifecycle/candidate/execution_candidate_manifest.py`, `builder_ii/execution_candidate_manifest_cli.py`, `tests/test_execution_candidate_manifest.py`; commands: `builder-hitl candidate-manifest/validate-candidate-manifest` | Manifest validates intent, rollback requirements, verification requirements, and command previews; it never activates. Missing executor. | B1 |
| HITL-approved verification execution | OPERATIONALLY_VERIFIED | `builder_ii/lifecycle/candidate/verification_execution_plan.py`, `builder_ii/lifecycle/candidate/verification_execution_approval.py`, `builder_ii/lifecycle/candidate/verification_execution_receipt.py`, `builder_ii/lifecycle/candidate/verification_execution_runner.py`, `builder_ii/governance/ledger/verification_execution_ledger.py`, `builder_ii/verification_execution_plan_cli.py`, `builder_ii/governance/hitl/hitl_command_execution.py`, `builder_ii/governance/hitl/hitl_execution_records.py`, `builder_ii/governance/hitl/hitl_verification_candidate.py`, `builder_ii/hitl_execution_cli.py`, `tests/test_verification_execution_plan.py`, `tests/test_verification_execution_plan_cli.py`, `tests/test_verification_execution_approval.py`, `tests/test_verification_execution_approval_cli.py`, `tests/test_verification_execution_approval_authority.py`, `tests/test_verification_execution_receipt.py`, `tests/test_verification_execution_receipt_cli.py`, `tests/test_verification_execution_runner.py`, `tests/test_verification_execution_ledger.py`, `tests/test_hitl_command_execution.py`, `tests/test_hitl_execution_records.py`, `tests/test_hitl_verification_candidate.py`; commands: `builder-verify plan/validate-plan/approve-plan/validate-approval/validate-receipt/run-approved`, `builder-ledger index-receipt/query-receipts/validate-receipts/reconstruct-receipts`, `builder-hitl request/receipt/validate` | B1.1 adds a digest-stable passive verification execution plan artifact only. B1.2 adds a digest-bound HITL approval artifact only. B1.3A/B add a receipt contract and bounded `platform_status` runner. B1.4A/B/C/D add passive verification ledger indexing, query, integrity reporting, reconstruction reporting, and closure docs. Broader execution profiles, live read authority, patch authority, model/MCP/Goose/deepagents runtime, and B2 write authority remain unpromoted. | B2.0 |
| HITL patch proposal | OPERATIONALLY_VERIFIED | `builder_ii/governance/hitl/hitl_patch_proposal.py`, `builder_ii/cli/hitl_patch_cli.py`; commands: `builder-hitl propose-patch` | Patch proposal artifact with digest binding is operational. | B4 |
| HITL patch application | OPERATIONALLY_VERIFIED | `builder_ii/governance/hitl/hitl_patch_apply.py`, `builder_ii/cli/hitl_patch_cli.py`; commands: `builder-hitl apply-patch` | git apply lane requires clean repo, approval digest, verification receipt; emits rollback bundle and failure receipts. No commit/push. | B4 |
| rollback execution | OPERATIONALLY_VERIFIED | `builder_ii/governance/hitl/hitl_patch_apply.py`, `builder_ii/lifecycle/candidate/rollback_artifacts.py`; commands: `builder-hitl rollback` | Rollback uses `git apply -R` on `forward_patch_for_reverse_apply.patch` with pre/post status digest proof. | B4 |
| postflight verification | OPERATIONALLY_VERIFIED | `builder_ii/lifecycle/candidate/execution_postflight_records.py`, `builder_ii/lifecycle/candidate/verification_execution_runner.py`; commands: `builder-verify run-approved` | Postflight records generated from bounded approved verification runs and patch apply lane. | B1.5 |
| Goose setup | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/adapters/goose/goose_setup.py` now provides passive setup metadata and redirect payloads; `builder_ii/cli.py setup/start`; tests cover launcher/setup | R1.4 removes legacy setup writes from `builder setup`, but Goose runtime promotion, recipe execution receipts, and governed runtime activation remain missing. | B4 after R0/B3 |
| Goose readonly runtime | MERGED_BUT_NOT_OPERATIONAL | Passive governed surface: `builder_ii/adapters/goose/goose_session.py`, `goose_readonly.py`, `goose_inspection.py`, `goose_cli.py`, `tests/test_goose_readonly.py`, `tests/test_goose_inspection.py`; legacy launcher: `builder_ii/adapters/goose/goose_launcher.py`; commands: `builder-goose manifest/readonly-audit/inspect-readonly/validate-*` | Governed CLI does not start Goose. Legacy `launch_goose_session` can start Goose but not under receipts/ledger/no-mutation proof. | B4 |
| Goose command proposals | PASSIVE_FOUNDATION | `builder_ii/adapters/goose/goose_command_proposal.py`, `tests/test_goose_command_proposal.py`; command: `builder-goose propose-command` | Proposal records require approval and say `executed=False`; no execution envelope or receipt. | B1/B4 |
| deepagents policy/readiness | PASSIVE_FOUNDATION | `builder_ii/adapters/deepagents/deepagents_policy.py`, `deepagents_readiness.py`, `deepagents_bridge.py`, `tests/test_deepagents_policy.py`, `tests/test_deepagents_readiness.py`, `tests/test_deepagents_bridge.py`; commands: `builder-deepagents policy/readiness/validate*` | Policy/readiness may inspect import metadata but construct no agents and grant no runtime authority; the separately approved native caller consumes a passing readiness gate. | B5 |
| deepagents passive work artifacts | PASSIVE_FOUNDATION | `builder_ii/adapters/deepagents/deepagents_work_artifacts.py`, `builder_ii/deepagents_cli.py`, `tests/test_deepagents_work_artifacts.py`; commands: `builder-deepagents work-plan/assign-subagent/record-result/review-result/request-human-gate/record-blocked-action/proposal-result/validate-work-artifact` | Work plan, assignment, result, review, gate, blocked-action, and proposal-result artifacts all deny model/tool/shell/Goose/deepagents/MCP/network/writes. | B5 |
| deepagents runtime/subagents | OPERATIONALLY_VERIFIED | `builder_ii/adapters/deepagents/deepagents_execution.py`, `builder_ii/adapters/deepagents/native_runtime.py`, `builder_ii/adapters/deepagents/native_artifacts.py`, `builder_ii/cli/deepagents_cli.py`, ...; commands: builder-deepagents execution-candidate, builder-deepagents approve-candidate, builder-deepagents run-approved, builder-deepagents resume-approved | `protocol_fake` is structural evidence only. The bounded native path uses official `create_deep_agent`, WRP obligations, one `ModelExecutionGateway` adapter, governed tools, digest-bound persistence, mandatory HITL interrupt/resume, and parent/child evidence; no target-repo, shell, Git, filesystem, direct-provider, or agent-output-quality claim. | B6 |
...
| release proof/quality gates | PASSIVE_FOUNDATION | `scripts/verify_v0_release.py`, `docs/RELEASE_PROOF.md`, `builder_ii/validation/quality_gates.py`, `builder_ii/quality_cli.py`, tests pass; commands: `builder-quality plan/validate`, proof harness | Proof harness proves passive artifact chain and no target mutation. It does not prove operational runtime. Quality gates are plans, not runners. | R0 then B1 |
| command authority as runtime gate | OPERATIONALLY_VERIFIED | `builder_ii/governance/authority/`, `docs/COMMAND_AUTHORITY.md`; commands: `builder-verify run-approved` and other authority-bearing lanes call `enforce_command_authority()` | Central fail-closed gate is operational for promoted lanes; `builder-hitl run-command` is fail-closed/unpromoted. Full legacy CLI wrapper interception remains partial. | B1.5 |
| docs truth enforcement | MERGED_BUT_NOT_OPERATIONAL | `docs/PLATFORM_COMPLETION_AUDIT.md`, `docs/FOUNDATION_STATUS.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, `tests/test_platform_completion_audit.py`, `tests/test_foundation_status.py` | Existing tests lock phrases like "foundation is complete"; no test compares doc claims against capability states. README stale line says model routing RFC exists although implementation exists. | R0 |
...
| `docs/PLATFORM_COMPLETION_AUDIT.md` | "v0 release" and "full governed artifact platform is built, tested, and proven" | confusing wording | The same paragraph says runtime execution remains ungated, but "full platform" can be read as operational platform. | Add "passive artifact platform" everywhere and link to R0 matrix. |
...
| `docs/plan/OPEN_SOURCE_V1_COMPLETION_PLAN.md` | "artifact foundation is complete" and "complete through PR #..." | confusing wording | The plan is future-looking, but "complete" is overloaded across passive and operational states. | Convert history sections to state labels. |
| `docs/HITL_VERIFICATION_CANDIDATE.md` | "Goal 2/3/4 complete" | harmless wording if scoped | It repeatedly says passive and candidate-only. | Add "PASSIVE_FOUNDATION" labels. |
...
- No versioned builder-II config schema exists. `builder_ii/core/config.py` loads `.env` plus process env and still exposes legacy `CORE_*` names as the primary operator knobs.
...
- No B1 verification runner exists. `builder_ii/core/harness.py` runs legacy CORE verification through `subprocess.run` without envelope digest, bounded capture, timeout, env allowlist, git mutation detection, approval artifact, receipt artifact, postflight, or ledger event.
- `builder_ii/governance/hitl/hitl_execution_records.py` creates request and `NOT_EXECUTED` receipt templates only.
- `builder_ii/governance/hitl/hitl_verification_candidate.py` constrains intent but explicitly sets `executes_now=False`.

### Missing read authority

- `builder_ii/core/readonly_inspection_reports.py` can hash explicit files and `builder_ii/adapters/goose/goose_inspection.py` can record bounded metadata, but there is no unified target-bound read authority with mandatory allowlist, budgets, secret/path traversal guards, artifact-root separation, content receipt policy, and ledger event.
- Legacy context path `builder_ii/core/context_pack.py` and `builder-context` remain operator-managed external scan surfaces.

### Missing write/patch authority

- `builder_ii/governance/hitl/hitl_patch_proposal.py` is design-only and forbids patch application.
...
Files likely touched: `builder_ii/core/platform_completion_audit.py`, `builder_ii/platform_status_cli.py`, `pyproject.toml`, `builder_ii/governance/authority/`, `docs/COMMAND_AUTHORITY.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, `docs/FOUNDATION_STATUS.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, `README.md`, `tests/test_platform_completion_truth.py`, `tests/test_docs_truth_enforcement.py`, `tests/test_command_authority.py`.
...
R1.1 files touched: `builder_ii/core/config.py`, new `builder_ii/core/config_schema.py`, new `builder_ii/core/config_sources.py`, new `builder_ii/lifecycle/setup/setup_plan.py`, new `builder_ii/config_cli.py`, new `builder_ii/setup_cli.py`, `builder_ii/governance/authority/`, `pyproject.toml`, `README.md`, `docs/COMMAND_AUTHORITY.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, new `docs/CONFIG_ONBOARDING.md`, tests.
...
R1.2 files touched: new `builder_ii/lifecycle/setup/setup_overlay.py`, new `builder_ii/lifecycle/setup/setup_rollback.py`, updated `builder_ii/lifecycle/setup/setup_plan.py`, `builder_ii/setup_cli.py`, `builder_ii/governance/authority/`, `builder_ii/core/platform_completion_audit.py`, docs, and tests.
...
Files likely touched: `builder_ii/lifecycle/candidate/verification_execution_plan.py`, `builder_ii/verification_execution_plan_cli.py`, `builder_ii/governance/authority/`, docs/tests.
...
Files likely touched: `builder_ii/lifecycle/candidate/verification_execution_approval.py`, `builder_ii/verification_execution_plan_cli.py`, `builder_ii/governance/ledger/artifact_index_records.py`, `builder_ii/core/artifact_chain_verification.py`, `builder_ii/governance/authority/`, docs/tests.
...
Files likely touched: `builder_ii/governance/ledger/event_ledger.py`, `workflow_orchestrator.py`, `artifact_chain_verification.py`, docs/tests.
...
Files likely touched: `builder_ii/governance/hitl/hitl_patch_proposal.py`, `builder_ii/governance/hitl/hitl_patch_apply.py`, `builder_ii/lifecycle/candidate/rollback_artifacts.py`, `builder_ii/lifecycle/candidate/execution_postflight_records.py`, `builder_ii/governance/authority/`, docs/tests.
...
Files likely touched: `builder_ii/governance/authority/readonly_authority.py`, `builder_ii/core/readonly_inspection_reports.py`, `builder_ii/adapters/goose/goose_inspection.py`, `builder_ii/governance/ledger/event_ledger.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/adapters/goose/goose_runtime_harness.py`, `goose_session.py`, `goose_readonly.py`, `goose_launcher.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/adapters/deepagents/deepagents_runtime.py`, `deepagents_policy.py`, `deepagents_work_artifacts.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/routing/model_execution_gateway.py`, `model_client_registry.py`, `model_routing_policy.py`, `direct_chat.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/core/mcp_policy.py`, `builder_ii/core/tool_invocation_gateway.py`, `tool_registry.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/core/artifact_memory.py`, `builder_ii/memory_cli.py`, `handoff_notes.py`, `handoff_bundle_records.py`, command authority, docs/tests.
...
Files likely touched: `builder_ii/lifecycle/setup/operator_status.py`, `builder_ii/lifecycle/setup/operator_next.py`, `builder_ii/platform_status_cli.py`, docs/tests.
...
1. Add `builder_ii/core/platform_completion_audit.py`.
...
5. Reword `docs/FOUNDATION_STATUS.md`, `docs/PLATFORM_COMPLETION_AUDIT.md`, and `README.md`:
