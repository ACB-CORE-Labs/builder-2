...
| config schema | DESIGN_ONLY | `builder_ii/config.py`, `tests/test_config_models.py`, `.env.example`; command: none dedicated | Settings are Python/env parsing, not a governed builder-II config artifact. Missing versioned schema, validator, redaction rules, source refs, target/artifact roots, capability defaults, and migration path away from legacy `CORE_*` names. | R1 |
| config source precedence | DESIGN_ONLY | `builder_ii/config.py` uses `load_dotenv(..., override=False)` and process env; README setup docs; tests require explicit `CORE_REPO_PATH=.` in this worktree | Precedence is implicit and fragmented across env, `.env`, CLI defaults, profile artifacts, and generated Goose files. No artifact says which source wins for target repo, artifact root, active profiles, model/backend, or capability state. | R1 |
| interactive setup wizard | NOT_STARTED | README has manual `.env` and `builder setup`; command: none | No wizard plans target repo, artifact root, profile choices, model/backend, Goose writes, recipes, skills, and disabled/passive/operational capabilities before apply. | R1 |
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/cli.py` setup/doctor/status, `builder_ii/goose_setup.py`, `tests/test_goose_setup.py`; commands: `builder setup`, `builder doctor`, `builder status` | Existing setup writes config/hints/skills but lacks a canonical plan/apply/validate contract, setup receipt, rollback artifact, source precedence record, and authority matrix. | R1 |
| Goose config overlay/rollback | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/goose_setup.py` writes `~/.config/goose/config.yaml`; `tests/test_goose_setup.py`, `tests/scenarios/test_config_to_goose_projection_flow.py` | Merge behavior exists, but no overlay diff, prior config snapshot, rollback artifact, approved write boundary, or audit receipt. | R1 |
...
| skill generator/installer/validator | MERGED_BUT_NOT_OPERATIONAL | `.agents/skills/`, `builder_ii/goose_setup.py`, `tests/test_goose_setup.py`; command: `builder setup` | Skill installer copies into the target repo. Missing generated install plan, digest-bound validation, setup receipt, rollback, and target-scoped approval. | R1 |
...
| setup receipt + rollback artifact | NOT_STARTED | Generic records exist in `builder_ii/receipt_records.py` and `builder_ii/rollback_artifacts.py`; no setup-specific tests or command | No setup preflight snapshot, setup plan digest, changed-path list, prior-config snapshot, rollback plan, rollback receipt, ledger event, or replay/audit binding. | R1 |
...
| Goose setup | MERGED_BUT_NOT_OPERATIONAL | Legacy `builder_ii/goose_setup.py` writes Goose config/hints/skills and validates recipes; `builder_ii/cli.py setup/start`; tests cover launcher/setup | Operational setup exists as Tier 2 operator-managed behavior, not governed runtime promotion. It writes outside target artifact-only lane and can use subprocesses. | B4 after R0/B3 |
...
| notes/handoff artifacts | PASSIVE_FOUNDATION | `builder_ii/notes_cli.py`, `handoff_artifacts.py`, `handoff_notes.py`, `handoff_bundle_records.py`, `tests/test_handoff_notes.py`, `tests/test_handoff_bundle_records.py`; commands: `builder-notes handoff/validate`, `builder-handoff` | Handoffs summarize and reference evidence; they do not mutate a memory store or prove execution. | B8 |
| artifact memory | DESIGN_ONLY | `docs/plan/ARTIFACT_MEMORY_RFC.md`; command: none | No memory atom schema, memory index, reconstruction artifact, staleness policy, searchable handoffs, or mutation approval. | B8 |
| operator quickstart/golden path | PASSIVE_FOUNDATION | `docs/OPERATOR_QUICKSTART.md`, `builder_ii/workflow_orchestrator.py`, `tests/test_prepare_package_quickstart.py`, `tests/test_workflow_ledger.py`; commands: `builder-session *`, `builder-workflow *` | Quickstart is a passive package lane. No one coherent operator path from target registration through read, plan, approve, verify, patch, rollback, handoff. | B9 |
...
| `README.md` and `builder setup` docs | Productized setup language around `.env`, Goose config, skills, and doctor/status | confusing wording | Current setup can mutate user Goose config and target repo support files, but there is no setup plan, receipt, rollback, or canonical source precedence. | R1 must split legacy operator-managed setup from the governed onboarding kernel. |
...
- `builder setup` can write Goose config, `.goosehints`, session context, and skill installs without setup receipt, rollback artifact, approved write boundary, or ledger event.
...
- `builder_ii/hitl_patch_spec.py` is design-only and forbids patch application.
...
- Artifact memory is an RFC; no memory atom, index, reconstruction artifact, staleness policy, searchable handoffs, or mutation approval path exists.
...
Goal: establish canonical, auditable, reversible platform/target/runtime configuration before any execution authority is promoted.

Files likely touched: `builder_ii/config.py`, new `builder_ii/config_schema.py`, new `builder_ii/config_sources.py`, new `builder_ii/setup_plan.py`, new `builder_ii/setup_receipts.py`, new `builder_ii/setup_cli.py`, `builder_ii/goose_setup.py`, `builder_ii/target_profiles.py`, `builder_ii/agent_profiles.py`, `builder_ii/verification_profiles.py`, `builder_ii/deepagents_policy.py`, `builder_ii/command_authority.py`, `pyproject.toml`, `README.md`, `docs/OPERATOR_QUICKSTART.md`, new `docs/CONFIG_ONBOARDING.md`, tests.

State change: config schema `DESIGN_ONLY` -> `PASSIVE_FOUNDATION`; config source precedence `DESIGN_ONLY` -> `PASSIVE_FOUNDATION`; interactive setup wizard `NOT_STARTED` -> `PASSIVE_FOUNDATION`; non-interactive setup/apply/validate `MERGED_BUT_NOT_OPERATIONAL` -> `PASSIVE_FOUNDATION`; Goose config overlay/rollback `MERGED_BUT_NOT_OPERATIONAL` -> `PASSIVE_FOUNDATION`; recipe generator/wizard `ARTIFACT_ONLY` -> `PASSIVE_FOUNDATION`; skill generator/installer/validator `MERGED_BUT_NOT_OPERATIONAL` -> `PASSIVE_FOUNDATION`; target/agent/verification/deepagents setup wizard rows `NOT_STARTED` -> `PASSIVE_FOUNDATION`; setup receipt + rollback artifact `NOT_STARTED` -> `PASSIVE_FOUNDATION`.

Command surface changes: add `builder-config schema/render/validate/resolve/doctor`; add `builder-setup plan/apply/rollback/status`; optionally add `builder-setup wizard` as an interactive facade over the same plan schema.

Artifact kinds: `builder_ii.config_schema`, `builder_ii.config_source_resolution`, `builder_ii.setup_plan`, `builder_ii.goose_config_overlay`, `builder_ii.recipe_install_plan`, `builder_ii.skill_install_plan`, `builder_ii.profile_creation_plan`, `builder_ii.setup_receipt`, `builder_ii.setup_rollback_plan`, `builder_ii.setup_rollback_receipt`.

Command authority changes: plan/validate/status are Tier 1 artifact-only commands. Apply/rollback are Tier 2 operator-managed setup writes with explicit target/user-config path allowlists, receipt emission, and no shell/model/tool/runtime escalation. R1 must not start Goose, models, deepagents, MCP, or arbitrary commands.

Tests required: config schema validation, env/`.env`/CLI/profile precedence, redaction of secrets, canonical target root and artifact root checks, artifact root outside target source paths, Goose overlay diff and prior snapshot, setup dry-run no-mutation, setup apply declared-path mutation only, rollback restores prior state, idempotent repeated apply, recipe/skill/profile wizard plan validation, disabled capability map, command authority registry entries, docs truth integration.

Docs required: config source precedence, setup plan/apply/rollback semantics, legacy `CORE_*` compatibility boundary, Goose overlay/rollback policy, wizard operator guide, capability defaults, no-runtime guarantee.
...
Rollback/no-mutation proof: `builder-setup plan` and `builder-setup validate` emit no-mutation proof. `builder-setup apply` writes only declared setup paths and emits a rollback plan plus prior-state snapshot. `builder-setup rollback` emits a rollback receipt and postflight path diff.

Acceptance criteria: `CORE_REPO_PATH=. uv run pytest -q`; setup plan/apply/rollback scenario tests pass in temp dirs; R0 matrix includes all config/onboarding rows; no setup command starts Goose, models, deepagents, MCP, or shell execution; generated docs and command authority registry agree.
