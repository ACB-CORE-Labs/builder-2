...
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/setup_apply.py`, `builder_ii/setup_rollback_execute.py`, `builder_ii/cli/setup_cli.py`; commands: `builder-setup apply`, `builder-setup rollback`, plus passive plan/overlay/rollback-snapshot chain | `builder-setup apply` and `builder-setup rollback` are digest-bound STATE_ENABLED setup mutation lanes; legacy `builder setup` fails closed. Passive plan chain and ambient Goose/runtime promotion remain outside this row. | R1 |
...
| setup receipt + rollback artifact | PASSIVE_FOUNDATION | `builder_ii/setup_apply.py`, `builder_ii/setup_receipt.py`, `builder_ii/setup_rollback_execute.py`, `builder_ii/setup_rollback_receipt.py`; commands: `builder-setup apply`, `builder-setup rollback`, `builder-setup rollback-snapshot` | R1.3A/B provide digest-bound setup apply receipts and setup rollback execution for eligible declared paths. Ledger event and full replay binding for setup lanes remain partial. | R1 |
