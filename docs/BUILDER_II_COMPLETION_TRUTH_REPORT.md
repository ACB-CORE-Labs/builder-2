...
| deepagents runtime/subagents | OPERATIONALLY_VERIFIED | `builder_ii/adapters/deepagents/deepagents_runtime.py`, `builder_ii/cli/deepagents_cli.py`; commands: `builder-deepagents run-plan`, `builder-deepagents collect-results` | deepagents runtime is operationally verified with subagent execution receipts and proposal-only results; no autonomous writes or hidden tool authority. | B6 |
