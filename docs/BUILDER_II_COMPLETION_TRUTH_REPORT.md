...
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/cli.py` setup/doctor/status, `builder_ii/goose_setup.py`, `builder_ii/setup_plan.py`, `builder_ii/setup_cli.py`, `tests/test_goose_setup.py`, `tests/test_setup_plan.py`, `tests/test_config_setup_cli.py`; commands: `builder setup`, `builder-setup plan/validate-plan` | R1.1 adds passive setup plan and validate-plan artifacts only. Existing setup still writes config/hints/skills, and governed setup apply, setup receipt, rollback artifact, and write authority are missing. | R1 |
| Goose config overlay/rollback | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/goose_setup.py` writes `~/.config/goose/config.yaml`; `builder_ii/setup_plan.py` describes Goose target paths passively; `tests/test_goose_setup.py`, `tests/scenarios/test_config_to_goose_projection_flow.py` | Legacy merge behavior exists. R1.1 can only describe Goose config and recipe paths; no overlay diff, prior config snapshot, rollback artifact, approved write boundary, or audit receipt. | R1 |
