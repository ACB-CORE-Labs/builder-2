...
| setup receipt + rollback artifact | PASSIVE_FOUNDATION | `builder_ii/setup_apply.py`, `builder_ii/setup_receipt.py`, `builder_ii/setup_rollback_execute.py`, `builder_ii/setup_rollback_receipt.py`; commands: `builder-setup apply`, `builder-setup rollback`, `builder-setup rollback-snapshot` | R1.3A/B add digest-bound setup apply receipts and setup rollback execution for declared paths; the row stays passive-foundation pending an R1 closure audit. Ledger event and full replay binding remain partial. | R1 |
| model registry | OPERATIONALLY_VERIFIED | `builder_ii/model_client_registry.py`, `builder_ii/model_execution_gateway.py`; commands: `builder-model-policy`, `builder-model call` | Governed registry under execution gateway with policy bounds. Legacy `builder ask/start` remain operator-managed helpers outside the canonical lane. | B7 |
| model routing | OPERATIONALLY_VERIFIED | `builder_ii/model_routing_policy.py`, `builder_ii/model_policy_cli.py`, `builder_ii/model_execution_gateway.py`; commands: `builder-model-policy`, `builder-model call` | Routing recommendations consumed by governed gateway; cloud/external calls require explicit policy/approval. | B7 |
| model/provider execution | OPERATIONALLY_VERIFIED | `builder_ii/model_execution_gateway.py`, `builder_ii/cli/model_cli.py`; commands: `builder-model call`, `builder-model standalone-call` (test/offline only) | Gateway enforces policy, registry, token caps, secret scan, envelope/receipt; `builder-model call` is ledger-bound operational lane. `standalone-call` is not ledger-bound proof. | B7 |
...
| MCP/tool invocation | PASSIVE_FOUNDATION | `builder_ii/mcp_policy.py`, `builder_ii/tool_invocation_gateway.py`; commands: `builder-mcp inventory/policy/call`, `builder-tools invoke` | MCP inventory/policy/call surfaces exist; live MCP server execution remains unpromoted/stub-limited. Low-risk deterministic tool stubs are `OPERATIONALLY_VERIFIED` via `low-risk tool invocation` row. | B7 |
...
| command authority as runtime gate | OPERATIONALLY_VERIFIED | `builder_ii/command_authority.py`, `docs/COMMAND_AUTHORITY.md`; commands: `builder-verify run-approved` and other authority-bearing lanes call `enforce_command_authority()` | Central fail-closed gate is operational for promoted lanes; `builder-hitl run-command` is fail-closed/unpromoted. Full legacy CLI wrapper interception remains partial. | B1.5 |
