...
| non-interactive setup/apply/validate | OPERATIONALLY_VERIFIED | `builder_ii/setup_apply.py`, `builder_ii/setup_rollback_execute.py`, `builder_ii/cli/setup_cli.py`; commands: `builder-setup apply`, `builder-setup rollback`, plus passive plan/overlay/rollback-snapshot chain | Digest-bound STATE_ENABLED setup apply/rollback are operationally verified; legacy `builder setup` fails closed. Passive plan/overlay validate chain remains passive scope. | R1 |
...
| setup receipt + rollback artifact | OPERATIONALLY_VERIFIED | `builder_ii/setup_apply.py`, `builder_ii/setup_receipt.py`, `builder_ii/setup_rollback_execute.py`, `builder_ii/setup_rollback_receipt.py`; commands: `builder-setup apply`, `builder-setup rollback`, `builder-setup rollback-snapshot` | R1.3A/B digest-bound setup apply receipts and setup rollback execution are operationally verified for declared paths. Ledger event and full replay binding remain partial. | R1 |
