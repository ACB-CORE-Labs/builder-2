...
| non-interactive setup/apply/validate | MERGED_BUT_NOT_OPERATIONAL | `builder_ii/cli.py` setup/doctor/status, `builder_ii/goose_setup.py`, `builder_ii/setup_plan.py`, `builder_ii/setup_overlay.py`, `builder_ii/setup_rollback.py`, `builder_ii/setup_cli.py`, `tests/test_goose_setup.py`, `tests/test_setup_plan.py`, `tests/test_setup_overlay.py`, `tests/test_setup_rollback.py`, `tests/test_config_setup_cli.py`; commands: `builder setup`, `builder-setup plan/validate-plan/overlay-plan/validate-overlay-plan/rollback-snapshot/validate-rollback-snapshot` | R1.2 adds passive setup plan, overlay plan, rollback snapshot, and validators only. Existing setup still writes config/hints/skills, and governed setup apply, setup receipt, rollback execution, and write authority are missing. | R1 |
...
| skill generator/installer/validator | MERGED_BUT_NOT_OPERATIONAL | `.agents/skills/`, `builder_ii/setup_overlay.py`, `builder_ii/goose_setup.py`, `tests/test_setup_overlay.py`, `tests/test_goose_setup.py`; commands: `builder-setup overlay-plan`, legacy `builder setup` | R1.2 adds passive install-plan entries with source/destination digests and conflict notes. Operational skill copying, setup receipt, rollback execution, and target-scoped approval are missing. | R1 |
