...
Use `builder-context` to create task-scoped context artifacts before Goose sessions, direct asks, reviews, and handoffs.

```bash
builder-context pack --task "review Goose recipe wiring"
builder-context pack --changed --task "summarize current work"
builder-context pack --module builder_ii/goose_setup.py
builder-context pack --module builder_ii/context_pack.py --no-repomix
```
...
The Markdown file is a manifest showing the task, git status, selected files, and Repomix command. The XML file is generated by Repomix when `--no-repomix` is not used.
...
builder-context pack --changed --task "current work"
...
| context pack fails | Run `builder-context pack --no-repomix` to validate selection without Repomix. |
...
builder-tools list
builder-tools check --tier tier1
builder-context pack --changed --task "..."
bash scripts/install-tools.sh status
builder start --task "..."
